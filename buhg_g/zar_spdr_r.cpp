/*$Id: zar_spdr_r.c,v 1.20 2013/09/26 09:47:00 sasa Exp $*/
/*17.05.2018	24.11.2006	Белых А.И.	zar_spdr_r.c
...
sprintf(strsql,"%s %s",iceb_get_namesystem(),gettext("Распечатка списока работников с датой рождения"));
gtk_window_set_title(GTK_WINDOW(data.window),strsql);

g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_spdr_r_key_press),&data);
...
g_signal_connect(data.knopka,"clicked",G_CALLBACK(zar_spdr_r_v_knopka),&data);
...
g_idle_add((GSourceFunc)zar_spdr_r1,&data);
...
iceb_zagolov(gettext("Распечатка списка работников с датой рождения"),dn,mn,gn,dk,mk,gk,ff,data->window);
...
//  strzag(LINES-1,0,kolstr,++kolstr1);
  ...
  (ii1=iceb_u_sravmydat(dr,mr,gt,dk,mk,gk)) <= 0)
   {
    pol.new_plus(gettext("Мужчина"));
    if(row[5][0] == '1')
      pol.new_plus(gettext("Женщина"));

    vozr=gt-gr;
    
    sprintf(strsql,"%4s %-*.*s %02d.%02d.%d %7d %s\n",
    row[0],iceb_u_kolbait(30,row[1]),iceb_u_kolbait(30,row[1]),row[1],dr,mr,gr,vozr,pol.ravno());

    iceb_printw(strsql,data->buffer,data->view,"naim_shrift");

    fprintf(ff,"%4s %-*s %02d.%02d.%d %7d %s\n",
    row[0],iceb_u_kolbait(40,row[1]),row[1],dr,mr,gr,vozr,pol.ravno());
  }
 }
fprintf(ff,"\
-------------------------------------------------------------------------\n");
