/*$Id: l_zarpodr.c,v 1.25 2013/08/25 08:26:46 sasa Exp $*/
/*08.05.2015	22.06.2006	Белых А.И.	l_zarpodr.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarpodr_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
gtk_widget_set_tooltip_text(data.knopka[SFK3],gettext("Удалить неиспользуемые коды подразделений"));
gtk_widget_set_name(data.knopka[SFK3],iceb_u_inttochar(SFK3));
gtk_widget_show(data.knopka[SFK3]);

sprintf(bros,"F4 %s",gettext("Поиск"));
data.knopka[FK4]=gtk_button_new_with_label(bros);
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_zarpodr_knopka),&data);
...
fprintf(ff,"\x1B\%c%c",'[',8); /*Уменьшение высоты символа*/
