/*$Id: l_kasopr.c,v 1.27 2013/12/31 11:49:13 sasa Exp $*/
/*11.07.2015	18.01.2006	Белых А.И.	l_kasopr.c
Ввод и корректировка списка операций расхода для подсистемы "Учёт кассовых ордеров"
*/
#include <errno.h>
#include "buhg_g.h"
#include "l_kasop.h"
...
gboolean   kasopr_key_press(GtkWidget *widget,GdkEventKey *event,class kasopr_data *data);
void kasopr_vibor(GtkTreeSelection *selection,class kasopr_data *data);
void kasopr_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,class kasopr_data *data);
void  kasopr_knopka(GtkWidget *widget,class kasopr_data *data);
void kasopr_add_columns (GtkTreeView *treeview);
void kasopr_create_list(class kasopr_data *data);
int  kasopr_prov_row(SQL_str row,class kasopr_data *data);

int l_kasopr_v(iceb_u_str *kodzap,GtkWidget *wpredok);
int l_kasopr_p(class kasop_rek *datap,GtkWidget *wpredok);
int  kasopr_pvu(int met,const char *kod,GtkWidget *wpredok);
void kasopr_udzap(class kasopr_data *data);
void kasopr_unk(GtkWidget *wpredok);
void kasopr_rasp(class kasopr_data *data);

extern SQL_baza	bd;
 
int l_kasopr(int metka, //0-воод и корректировка 1-выбор
iceb_u_str *kod,iceb_u_str *naimk,GtkWidget *wpredok)
{
int gor=0;
int ver=0;

class kasopr_data data;
char bros[512];
data.metka_rr=metka;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(kasopr_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(kasopr_knopka),&data);
...
switch (knop)
 {
  case FK2:
    data->kodtv.new_plus("");
    l_kasopr_v(&data->kodtv,data->window);
    kasopr_create_list(data);
    return;  

  case SFK2:

    if(l_kasopr_v(&data->kodv,data->window) == 0)
...
//    printf("data->snanomer=%d\n",data->snanomer);
  ...
  case SFK3:
    kasopr_unk(data->window);
    kasopr_create_list(data);
    return;  

  case FK4:
    l_kasopr_p(&data->poi,data->window);
    kasopr_create_list(data);
    return;  
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(kasopr_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(kasopr_vibor),data);
...
sprintf(strsql,"select * from Kasop2 order by kod asc");
...
kasopr_add_columns (GTK_TREE_VIEW (data->treeview));
...
//printf("kasopr_vibor-%s %d\n",data->kodv.ravno(),data->snanomer);

}
/****************************/
/*Выбор строки по двойному клику мышкой*/
/**********************/
void kasopr_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class kasopr_data *data)
{
...
if(kasopr_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Kasop2 where kod='%s'",
data->kodv.ravno());
...
sprintf(strsql,"select kodop from Kasord where kodop='%s' and tp=2 limit 1",kod);
...
while(cur.read_cursor(&row) != 0)
 {
  if(kasopr_pvu(0,row[0],wpredok) != 0)
    continue;
  sprintf(strsql,"delete from Kasop2 where kod='%s'",row[0]);
...
sprintf(strsql,"kasopr%d.lst",getpid());
...
iceb_zagolov(gettext("Список расходных операций"),0,0,0,0,0,0,ff,data->window);
