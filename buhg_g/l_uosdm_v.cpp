/*$Id: l_uosdm_v.c,v 1.14 2013/09/05 07:24:33 sasa Exp $*/
/*01.03.2016	21.06.2006	Белых А.И.	l_uosdm_v.c
Ввод и корректировка справочника категорий работников
*/
#include "buhg_g.h"
#include "l_uosdm.h"
enum
...
gboolean   l_uosdm_v_key_press(GtkWidget *widget,GdkEventKey *event,class l_uosdm_v_data *data);
void  l_uosdm_v_knopka(GtkWidget *widget,class l_uosdm_v_data *data);
void    l_uosdm_v_vvod(GtkWidget *widget,class l_uosdm_v_data *data);
int l_uosdm_pk(const char *god,GtkWidget*);
int l_uosdm_zap(class l_uosdm_v_data *data);
void l_uosdm_kkvt(const char *kods,const char *kodn,GtkWidget *wpredok);
void  l_uosdm_v_e_knopka(GtkWidget *widget,class l_uosdm_v_data *data);
...
if(data.kod_gr.getdlinna() >  1)
 {
  ...
  sprintf(strsql,"select * from Uosdm where kod=%s",data.kod_gr.ravno());
  SQL_str row;
  SQLCURSOR cur;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosdm_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_uosdm_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_uosdm_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_EI],"clicked",G_CALLBACK(l_uosdm_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_EI], "activate",G_CALLBACK(l_uosdm_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_uosdm_v_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F10 %s",gettext("Выход"));
data.knopka[FK10]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK10],gettext("Завершение работы с меню без записи введенной информации"));
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_uosdm_v_knopka),&data);
...
//g_print("l_uosdm_v_e_knopka knop=%d\n",knop);
...
if(iceb_u_SRAV(data->kod_gr.ravno(),data->rk.kod.ravno(),0) != 0)
  if(l_uosdm_pk(data->rk.kod.ravno(),data->window) != 0)
     return(1);
short metkakor=0;
...
if(data->kod_gr.getdlinna() <= 1)
 {
  ...
  sprintf(strsql,"insert into Uosdm \
values (%d,'%s','%s',%d,%ld)",
...
if(metkakor == 1)
 l_uosdm_kkvt(data->kod_gr.ravno(),data->rk.kod.ravno(),data->window);
...
/**********************************/
int l_uosdm_pk(const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kod from Uosdm where kod=%s",kod);
...
void l_uosdm_kkvt(const char *skod,const char *nkod,GtkWidget *wpredok)
{
