/*$Id: iceb_l_grupmat_p.c,v 1.16 2013/08/25 08:27:05 sasa Exp $*/
/*28.02.2016	05.05.2004	Белых А.И.	iceb_l_grupmat_p.c
...
class iceb_l_grupmat_p_data
 {
  ...
  iceb_l_grupmat_p_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
void grupmat_p_clear(class iceb_l_grupmat_p_data *data);
void    grupmat_p_vvod(GtkWidget *widget,class iceb_l_grupmat_p_data *data);
void  grupmat_p_knopka(GtkWidget *widget,class iceb_l_grupmat_p_data *data);
gboolean   grupmat_p_key_press(GtkWidget *widget,GdkEventKey *event,class iceb_l_grupmat_p_data *data);

int iceb_l_grupmat_p(class grupmat_rek *rek_poi,GtkWidget *wpredok)
{
class iceb_l_grupmat_p_data data;
char strsql[512];
int gor=0,ver=0;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(grupmat_p_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(grupmat_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(grupmat_p_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(grupmat_p_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F4 %s",gettext("Очистить"));
data.knopka[FK4]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK4],gettext("Очистить меню от введеноой информации"));
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(grupmat_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(grupmat_p_knopka),&data);
