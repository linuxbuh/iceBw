/*$Id: l_poiupldok.c,v 1.15 2013/12/31 11:49:13 sasa Exp $*/
/*11.07.2015	25.03.2008	Белых А.И.	l_poiupldok.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(poiupldok_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(poiupldok_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(poiupldok_knopka),&data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(iceb_sql_readkey(strsql,&row1,&cur1,data->window) == 1)
   naim.new_plus(row1[0]);  
  sprintf(strsql,"%s %s",row[6],naim.ravno());
  ...
  if(iceb_sql_readkey(strsql,&row1,&cur1,data->window) == 1)
   naim.new_plus(row1[0]);  
  sprintf(strsql,"%s %s",row[5],naim.ravno());
...
//    l_upldok(strsql,data->nomdok.ravno(),data->kod_pod.ravno(),data->window);
    l_upldok3(strsql,data->nomdok.ravno(),data->kod_pod.ravno_atoi(),data->window);
    poiupldok_create_list(data);
    return;  
