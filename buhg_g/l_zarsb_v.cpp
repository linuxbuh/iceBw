/*$Id: l_zarsb_v.c,v 1.13 2013/08/25 08:26:47 sasa Exp $*/
/*07.07.2016	10.09.2009	Белых А.И.	l_zarsb_v.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarsb_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_zarsb_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_zarsb_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_zarsb_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_zarsb_v_knopka),&data);
...
if(data->kolzap == 0)
 {
  ...
  if(iceb_sql_readkey(strsql,data->window) > 0)
   {
    sprintf(strsql,"update Zarkh set kb=%d",data->rk.kod.ravno_atoi());
    iceb_sql_zapis(strsql,0,0,data->window);
   }
  else
   {
    ...
    if(kolstr > 0)
     while(cur.read_cursor(&row) != 0)
      {
       sprintf(strsql,"insert into Zarkh values(%s,%d,'%s',%d,%ld)",row[0],data->rk.kod.ravno_atoi(),row[1],iceb_getuid(data->window),time(NULL));
       iceb_sql_zapis(strsql,0,0,data->window);
      }  
   }  
 }
