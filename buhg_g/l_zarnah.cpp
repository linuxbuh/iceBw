/*$Id: l_zarnah.c,v 1.21 2013/08/25 08:26:46 sasa Exp $*/
/*31.05.2016	22.06.2006	Белых А.И.	l_zarnah.c
Работа со списком начислений
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_zarnah.h"
...
gboolean   l_zarnah_key_press(GtkWidget *widget,GdkEventKey *event,class l_zarnah_data *data);
void l_zarnah_vibor(GtkTreeSelection *selection,class l_zarnah_data *data);
void l_zarnah_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_zarnah_data *data);
void  l_zarnah_knopka(GtkWidget *widget,class l_zarnah_data *data);
void l_zarnah_add_columns (GtkTreeView *treeview);
void l_zarnah_udzap(class l_zarnah_data *data);
int  l_zarnah_prov_row(SQL_str row,class l_zarnah_data *data);
void l_zarnah_rasp(class l_zarnah_data *data);
int  l_zarnah_pvu(int,const char *kod,GtkWidget *wpredok);
void l_zarnah_create_list (class l_zarnah_data *data);

int l_zarnah_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_zarnah_p(class l_zarnah_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_zarnah_data *data);

extern SQL_baza  bd;

int   l_zarnah(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список начислений"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarnah_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_zarnah_knopka),&data);
...
renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Код табеля"), renderer,"text", COL_KOD_TAB,NULL);

renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Вид начисления"), renderer,"text", COL_VID_NAH,NULL);
...
renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Фонд зарплаты"), renderer,"text", COL_FZ,NULL);
...
//g_print("l_zarnah_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    l_zarnah_v(&data->kod_kat_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_zarnah_v(&data->kodv,data->window);
  ...
  case FK4:
    ...
    if(l_zarnah_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_zarnah_create_list(data);
...
if(l_zarnah_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Nash where kod=%s",data->kodv.ravno());
...
sprintf(strsql,"nah%d.lst",getpid());
...
iceb_zagolov(gettext("Список начислений"),0,0,0,0,0,0,ff,data->window);
...
fprintf(ff,gettext("\
Код |          Наименование        |Счёт   |К.т.|Вид начисления|Фонд зарплаты|Проводки|Дата и время запис.| Кто записал\n"));
/******
1234 12345678901234567890 1234567 1234 12345678901234 1234567890123 12345678
********/
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  //Вид начисления
  if(row[4][0] == '0')
   vid_nah.new_plus(gettext("Основная"));
  if(row[4][0] == '1')
   vid_nah.new_plus(gettext("Дополнительная"));
  if(row[4][0] == '2')
   vid_nah.new_plus(gettext("Прочая"));

  if(row[8][0] == '0')
   fond_z.new_plus(gettext("Входит"));
  else  
   fond_z.new_plus(gettext("Невходит"));

  //Проводки
  if(row[7][0] == '0')
   prov.new_plus(gettext("Нужно делать"));
  else  
   prov.new_plus(gettext("Не нужно делать"));

  fprintf(ff,"%-4s %-*.*s %-*s %-*s %-*.*s %-*.*s %-*.*s %s %s\n",
  row[0],
  iceb_u_kolbait(30,row[1]),iceb_u_kolbait(30,row[1]),row[1],
  iceb_u_kolbait(7,row[2]),row[2],
  iceb_u_kolbait(4,row[3]),row[3],
  iceb_u_kolbait(14,vid_nah.ravno()),iceb_u_kolbait(14,vid_nah.ravno()),vid_nah.ravno(),
  iceb_u_kolbait(13,fond_z.ravno()),iceb_u_kolbait(13,fond_z.ravno()),fond_z.ravno(),
  iceb_u_kolbait(8,prov.ravno()),iceb_u_kolbait(8,prov.ravno()),prov.ravno(),
...
int  l_zarnah_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[1024];

sprintf(strsql,"select knah from Zarp where knah=%s and prn='1' limit 1",kod);
...
sprintf(strsql,"select knah from Zarn1 where knah=%s and prn=1 limit 1",kod);

if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
    sprintf(strsql,"%s Zarn1",gettext("Удалить невозможно ! Есть записи в таблице"));
    iceb_menu_soob(strsql,wpredok);
   }
  return(1);
 }    

sprintf(strsql,"select kn from Zarsdo where kn=%s limit 1",kod);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_zarnah_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Nash where kod=%s",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
