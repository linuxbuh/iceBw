/*$Id: l_kaskcn.c,v 1.22 2013/12/31 11:49:13 sasa Exp $*/
/*08.05.2015	29.09.2006	Белых А.И.	l_kaskcn.c
Работа со списком кодов целевого назначения
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_kaskcn.h"
...
gboolean   l_kaskcn_key_press(GtkWidget *widget,GdkEventKey *event,class l_kaskcn_data *data);
void l_kaskcn_vibor(GtkTreeSelection *selection,class l_kaskcn_data *data);
void l_kaskcn_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_kaskcn_data *data);
void  l_kaskcn_knopka(GtkWidget *widget,class l_kaskcn_data *data);
void l_kaskcn_add_columns (GtkTreeView *treeview);
void l_kaskcn_udzap(class l_kaskcn_data *data);
int  l_kaskcn_prov_row(SQL_str row,class l_kaskcn_data *data);
void l_kaskcn_rasp(class l_kaskcn_data *data);
int  l_kaskcn_pvu(int,const char *kod,GtkWidget *wpredok);
void l_kaskcn_create_list (class l_kaskcn_data *data);

int l_kaskcn_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_kaskcn_p(class l_kaskcn_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_kaskcn_data *data);


extern SQL_baza  bd;

int   l_kaskcn(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список кодов целевого назначения"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_kaskcn_key_press),&data);
...
data.label_kolstr=gtk_label_new (gettext("Список кодов целевого назначения"));
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_kaskcn_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_kaskcn_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_kaskcn_vibor),data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_kaskcn_prov_row(row,data) != 0)
    continue;
...
l_kaskcn_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список кодов целевого назначения"));
...
//g_print("l_kaskcn_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_kaskcn_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_kaskcn_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_kaskcn_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_kaskcn_create_list(data);
...
if(l_kaskcn_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Kascn where kod=%s",
data->kodv.ravno());
...
sprintf(strsql,"kaskcn%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Список кодов целевого назначения"));
...
iceb_zagolov(gettext("Список кодов целевого назначения"),0,0,0,0,0,0,ff,data->window);
...
int  l_kaskcn_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kod from Kasop1 where kcn=%s",kod);
...
sprintf(strsql,"select kod from Kasop2 where kcn=%s",kod);
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
...
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
    iceb_u_str soob;
    sprintf(strsql,"%s Kascnsl",gettext("Удалить невозможно ! Есть записи в таблице"));
    soob.plus(strsql);
    iceb_menu_soob(&soob,wpredok);
   }
  return(1);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_kaskcn_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Kascn where kod=%s",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
