/*$Id: l_taxitel_p.c,v 1.5 2013/08/13 06:10:24 sasa Exp $*/
/*05.05.2009	22.06.2006	Белых А.И.	l_taxitel_p.c
...
void l_taxitel_p_clear(class l_taxitel_p_data *data);
void    l_taxitel_p_vvod(GtkWidget *widget,class l_taxitel_p_data *data);
void  l_taxitel_p_knopka(GtkWidget *widget,class l_taxitel_p_data *data);
gboolean   l_taxitel_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_taxitel_p_data *data);

extern char      *name_system;

int l_taxitel_p(class l_taxitel_rek *rek_poi,GtkWidget *wpredok)
{
class l_taxitel_p_data data;
char strsql[300];
data.rk=rek_poi;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_taxitel_p_key_press),&data);
...
GtkWidget *label=gtk_label_new(gettext("Телефон"));
gtk_box_pack_start (GTK_BOX (hbox[E_KOD]), label, FALSE, FALSE, 0);

data.entry[E_KOD] = gtk_entry_new();
gtk_box_pack_start (GTK_BOX (hbox[E_KOD]), data.entry[E_KOD], TRUE, TRUE, 0);
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_taxitel_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_taxitel_p_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_taxitel_p_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(l_taxitel_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_taxitel_p_knopka),&data);
...
//g_print("l_taxitel_p_knopka knop=%d\n",knop);
