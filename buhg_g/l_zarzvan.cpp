/*$Id: l_zarzvan.c,v 1.20 2013/08/25 08:26:47 sasa Exp $*/
/*08.05.2015	21.06.2006	Белых А.И.	l_zarzvan.c
Работа со списком званий
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_zarzvan.h"
...
class  l_zarzvan_data
 {
  ...
  iceb_u_str kodv;
  iceb_u_str naimv;
  iceb_u_str kod_zvan_tv; //только что введённая единица измерения
...
gboolean   l_zarzvan_key_press(GtkWidget *widget,GdkEventKey *event,class l_zarzvan_data *data);
void l_zarzvan_vibor(GtkTreeSelection *selection,class l_zarzvan_data *data);
void l_zarzvan_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_zarzvan_data *data);
void  l_zarzvan_knopka(GtkWidget *widget,class l_zarzvan_data *data);
void l_zarzvan_add_columns (GtkTreeView *treeview);
void l_zarzvan_udzap(class l_zarzvan_data *data);
int  l_zarzvan_prov_row(SQL_str row,class l_zarzvan_data *data);
void l_zarzvan_rasp(class l_zarzvan_data *data);
int  l_zarzvan_pvu(int,const char *kod,GtkWidget *wpredok);
void l_zarzvan_create_list (class l_zarzvan_data *data);

int l_zarzvan_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_zarzvan_p(class l_zarzvan_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_zarzvan_data *data);
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список званий"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarzvan_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_zarzvan_knopka),&data);
...
renderer = gtk_cell_renderer_text_new ();
gtk_tree_view_insert_column_with_attributes (GTK_TREE_VIEW (treeview),
-1,gettext("Сумма доплаты за звание"), renderer,"text", COL_SDZZ,NULL);
...
//g_print("l_zarzvan_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    l_zarzvan_v(&data->kod_zvan_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_zarzvan_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_zarzvan_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_zarzvan_create_list(data);
...
if(l_zarzvan_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Zvan where kod=%s",data->kodv.ravno());
...
sprintf(strsql,"zvan%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Список званий"));
...
iceb_u_zagolov(gettext("Список званий"),0,0,0,0,0,0,iceb_get_pnk("00",1,data->window),ff);
...
fprintf(ff,gettext("\
Код |    Наименование    |Сумма доплаты за звание|Дата и время запис.| Кто записал\n"));
/*********
1234 12345678901234567890 12345678901234567890123 
*********/
...
int  l_zarzvan_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
char strsql[512];
sprintf(strsql,"select kod from Zvan");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_zarzvan_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Zvan where kod=%s",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
