/*$Id: l_taxitel.c,v 1.9 2013/08/13 06:10:24 sasa Exp $*/
/*12.05.2015	30.05.2009	Белых А.И.	l_taxitel.c
Работа со списком телефонных номеров
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gboolean   l_taxitel_key_press(GtkWidget *widget,GdkEventKey *event,class l_taxitel_data *data);
void l_taxitel_vibor(GtkTreeSelection *selection,class l_taxitel_data *data);
void l_taxitel_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_taxitel_data *data);
void  l_taxitel_knopka(GtkWidget *widget,class l_taxitel_data *data);
void l_taxitel_add_columns (GtkTreeView *treeview);
void l_taxitel_udzap(class l_taxitel_data *data);
int  l_taxitel_prov_row(SQL_str row,class l_taxitel_data *data);
void l_taxitel_rasp(class l_taxitel_data *data);
int  l_taxitel_pvu(int,const char *kod,GtkWidget *wpredok);
void l_taxitel_create_list (class l_taxitel_data *data);

int l_taxitel_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_taxitel_p(class l_taxitel_rek *rek_poi,GtkWidget *wpredok);
...
int   l_taxitel(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",name_system,"Список телефонов");
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_taxitel_key_press),&data);
...
data.label_kolstr=gtk_label_new ("Список телефонов");
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_taxitel_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_taxitel_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_taxitel_vibor),data);
...
sprintf(strsql,"select * from Taxitel order by tel asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_taxitel_prov_row(row,data) != 0)
    continue;
  ...
  //Кто записал
  ss[COL_KTO].new_plus(iceb_u_kszap(row[2],0));
...
l_taxitel_add_columns (GTK_TREE_VIEW (data->treeview));
...
if(data->metka_poi == 1)
 {
  ...
  iceb_str_poisk(&zagolov,data->poisk.kod.ravno(),gettext("Телефон"));
  iceb_str_poisk(&zagolov,data->poisk.naim.ravno(),gettext("Адрес"));
...
column = gtk_tree_view_column_new_with_attributes (gettext("Телефон"),renderer,"text",COL_KOD,NULL);
...
renderer = gtk_cell_renderer_text_new ();
column = gtk_tree_view_column_new_with_attributes (gettext("Адрес"),renderer,"text",COL_NAIM,NULL);
gtk_tree_view_column_set_resizable(column,TRUE); /*Разрешение на изменение размеров колонки*/
...
//g_print("l_taxitel_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    if(l_taxitel_v(&data->kod_kat_tv,data->window) == 0)
      l_taxitel_create_list(data);
    return;  
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    if(l_taxitel_v(&data->kodv,data->window) == 0)
      l_taxitel_create_list(data);
    return;  
  ...
  case FK3:
    ...
    if(iceb_menu_danet(gettext("Удалить запись ? Вы уверены ?"),2,data->window) != 1)
     return;
    l_taxitel_udzap(data);
    l_taxitel_create_list(data);
    return;  
  

  case FK4:
    if(l_taxitel_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_taxitel_create_list(data);
...
class l_taxitel_data *data)
{
//printf("l_taxitel_v_row\n");
//printf("l_taxitel_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  g_signal_emit_by_name(data->knopka[SFK2],"clicked");
  return;
...
if(l_taxitel_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[300];

sprintf(strsql,"delete from Taxitel where tel='%s'",data->kodv.ravno());
...
iceb_u_zagolov("Список телефонов",0,0,0,0,0,0,iceb_get_pnk("00",0,data->window),ff);

if(data->metka_poi == 1)
 {
  if(data->poisk.kod.getdlinna() > 1)
   fprintf(ff,"%s:%s\n",gettext("Телефон"),data->poisk.kod.ravno());
  if(data->poisk.naim.getdlinna() > 1)
   fprintf(ff,"%s:%s\n",gettext("Адрес"),data->poisk.naim.ravno());
 }
...
fprintf(ff,"\
Номер телефона|   Адрес нахждения телефона   |Дата и время запис.| Кто записал\n");
/*
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  fprintf(ff,"%-14s %-*.*s %s %s\n",
  row[0],
  iceb_u_kolbait(30,row[1]),
  iceb_u_kolbait(30,row[1]),
  row[1],
  iceb_u_vremzap(row[3]),iceb_u_kszap(row[2],1));
...
int  l_taxitel_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[300];
