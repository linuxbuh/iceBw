/*$Id: l_uoshz.c,v 1.21 2013/12/31 11:49:15 sasa Exp $*/
/*08.05.2015	18.10.2007	Белых А.И.	l_uoshz.c
Работа со списком шифров затрат в учёте основных средств
*/
#include        <errno.h>
...
gboolean   l_uoshz_key_press(GtkWidget *widget,GdkEventKey *event,class l_uoshz_data *data);
void l_uoshz_vibor(GtkTreeSelection *selection,class l_uoshz_data *data);
void l_uoshz_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uoshz_data *data);
void  l_uoshz_knopka(GtkWidget *widget,class l_uoshz_data *data);
void l_uoshz_add_columns (GtkTreeView *treeview);
void l_uoshz_udzap(class l_uoshz_data *data);
int  l_uoshz_prov_row(SQL_str row,class l_uoshz_data *data);
void l_uoshz_rasp(class l_uoshz_data *data);
int  l_uoshz_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uoshz_create_list (class l_uoshz_data *data);

int l_uoshz_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uoshz_p(class l_uoshz_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uoshz_data *data);


extern SQL_baza  bd;

int   l_uoshz(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список шифров затрат"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uoshz_key_press),&data);
...
data.label_kolstr=gtk_label_new (gettext("Список шифров затрат"));
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uoshz_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uoshz_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uoshz_vibor),data);
...
sprintf(strsql,"select * from Uoshz order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uoshz_prov_row(row,data) != 0)
    continue;
...
l_uoshz_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список шифров затрат"));
...
//g_print("l_uoshz_knopka knop=%d\n",knop);
data->kl_shift=0; //0-отжата 1-нажата  

switch (knop)
 {
  case FK2:
    l_uoshz_v(&data->kod_podr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uoshz_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uoshz_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uoshz_create_list(data);
...
if(l_uoshz_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uoshz where kod='%s'",data->kodv.ravno());
...
sprintf(strsql,"hz%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Список шифров затрат"));
...
iceb_zagolov(gettext("Список шифров затрат"),0,0,0,0,0,0,ff,data->window);
...
int  l_uoshz_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
sprintf(strsql,"select hzt from Uosamor1 where hzt='%s' limit 1",kod);
...
char strsql[512];
sprintf(strsql,"select kod from Uoshz");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uoshz_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uoshz where kod='%s'",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
