/*$Id: m_vprov_nbs.c,v 1.25 2013/09/26 09:46:51 sasa Exp $*/
/*06.05.2015	05.01.2004	Белых А.И.	m_vprov_nbs.c
Ввод проводок для небалансовых счетов
*/
#include  <math.h>
#include  "buhg_g.h"
...
int vprov_nbs_zp(class vprov_nbs_data *data);
void   vregp(GtkWidget*);
void m_vprov_nbs_showprov(class vprov_nbs_data *data);
void  m_vprov_nbs_v_e_knopka(GtkWidget *widget,class vprov_nbs_data *data);

extern SQL_baza	bd;

void  m_vprov_nbs(const char *shet)
{
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(vprov_nbs_v_key_press),&data);
...
gtk_box_pack_start (GTK_BOX (vbox2), separator1, TRUE, TRUE, 0);

gtk_box_pack_start (GTK_BOX (vbox2), data.label_prov, TRUE, TRUE, 0);

gtk_box_pack_start (GTK_BOX (vbox2), separator2, TRUE, TRUE, 0);
...
g_signal_connect(data.entry[E_DEBET], "activate",G_CALLBACK(vprov_nbs_v_vvod),&data);
...
g_signal_connect(data.entry[E_KREDIT], "activate",G_CALLBACK(vprov_nbs_v_vvod),&data);
...
gtk_box_pack_start (GTK_BOX (hbox_enter[E_DATA]), data.knopka_enter, FALSE, FALSE, 0);
g_signal_connect(data.knopka_enter,"clicked",G_CALLBACK(m_vprov_nbs_v_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter,iceb_u_inttochar(E_DATA));
gtk_widget_set_tooltip_text(data.knopka_enter,gettext("Выбор даты"));
...
g_signal_connect(data.entry[E_DATA], "activate",G_CALLBACK(vprov_nbs_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOMENT], "activate",G_CALLBACK(vprov_nbs_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(vprov_nbs_v_knopka),&data);
...
g_signal_connect(data.knopka[FK3],"clicked",G_CALLBACK(vprov_nbs_v_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(vprov_nbs_v_knopka),&data);
...
g_signal_connect(data.knopka[FK5],"clicked",G_CALLBACK(vprov_nbs_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(vprov_nbs_v_knopka),&data);
...
datpr.new_plus(data.data.ravno(),sizeof(datpr)-1);
...
switch (knop)
 {
  case FK2:
    if(vprov_nbs_zp(data) == 0)
     {
      m_vprov_nbs_showprov(data);
      data->clear_zero1();
     }
    return;  
...
time(&vrem);
int kekv=0;
iceb_zapprov(-1,g,m,d,data->shet.ravno_filtr(),"",data->kontr_sh.ravno_filtr(),"","","","",deb,kre,data->koment.ravno_filtr(),1,0,vrem,0,0,0,0,kekv,data->window);
...
int m_vprov_nbs_menur(GtkWidget *wpredok)
{
...
/*Выбор строки*/
/**********************/
void m_vprov_nbs_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class vprov_nbs_data *data)
{
int nomr=0;
if((nomr=m_vprov_nbs_menur(data->window)) < 0)
 return;
...
void m_vprov_nbs_readstr(GtkTreeSelection *selection,class vprov_nbs_data *data)
{
GtkTreeModel *model;
GtkTreeIter  iter;
...
void m_vprov_nbs_add_columns(GtkTreeView *treeview)
{
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(m_vprov_nbs_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(m_vprov_nbs_readstr),data);
...
m_vprov_nbs_add_columns (GTK_TREE_VIEW (data->treeview));
