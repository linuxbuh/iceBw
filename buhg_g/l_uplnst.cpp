/*$Id: l_uplnst.c,v 1.4 2013/08/25 08:26:42 sasa Exp $*/
/*08.05.2015	20.04.2012	Белых А.И.	l_uplnst.c
Работа со списком видов норм списания топлива
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_uplnst.h"
...
gboolean   l_uplnst_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplnst_data *data);
void l_uplnst_vibor(GtkTreeSelection *selection,class l_uplnst_data *data);
void l_uplnst_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uplnst_data *data);
void  l_uplnst_knopka(GtkWidget *widget,class l_uplnst_data *data);
void l_uplnst_add_columns (GtkTreeView *treeview);
void l_uplnst_udzap(class l_uplnst_data *data);
int  l_uplnst_prov_row(SQL_str row,class l_uplnst_data *data);
void l_uplnst_rasp(class l_uplnst_data *data);
int  l_uplnst_pvu(int,const char *kod,GtkWidget *wpredok);
int l_uplnst_create_list (class l_uplnst_data *data);

int l_uplnst_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uplnst_p(class l_uplnst_rek *rek_poi,GtkWidget *wpredok);
void l_uplnst_unk(class l_uplnst_data *data);


extern SQL_baza  bd;

int   l_uplnst(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список видов норм списания топлива"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uplnst_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uplnst_knopka),&data);
...
if(l_uplnst_create_list(&data) != 0)
 return(1);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uplnst_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uplnst_vibor),data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uplnst_prov_row(row,data) != 0)
    continue;
...
l_uplnst_add_columns (GTK_TREE_VIEW (data->treeview));
...
//g_print("l_uplnst_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    if(l_uplnst_v(&data->kod_kat_tv,data->window) == 0)
      l_uplnst_create_list(data);
    return;  

  case SFK2:
    if(data->kolzap == 0)
      return;
    if(l_uplnst_v(&data->kodv,data->window) == 0)
      l_uplnst_create_list(data);
    return;  
  ...
  case FK4:
    if(l_uplnst_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uplnst_create_list(data);
...
if(l_uplnst_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uplnst where kod='%s'",data->kodv.ravno());
...
sprintf(strsql,"uplnst%d.lst",getpid());
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  fprintf(ff,"%-*s %-*.*s %s %s\n",
  iceb_u_kolbait(6,row[0]),
  row[0],
  iceb_u_kolbait(30,row[1]),iceb_u_kolbait(30,row[1]),row[1],
  iceb_u_vremzap(row[3]),iceb_kszap(row[2],data->window));

  for(int ii=30; ii < iceb_u_strlen(row[1]); ii+=30)
   fprintf(ff,"%6s %-*.*s\n",
   "",
   iceb_u_kolbait(30,iceb_u_adrsimv(ii,row[1])),
   iceb_u_kolbait(30,iceb_u_adrsimv(ii,row[1])),
...
int  l_uplnst_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
char strsql[512];
sprintf(strsql,"select kod from Uplnst");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uplnst_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uplnst where kod='%s'",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
