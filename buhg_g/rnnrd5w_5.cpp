...
/*12.07.2015	03.04.2008	Белых А.И.	rnnrd5w_5.c
...
void rnnrd5_endras(class rnn_d5 *dod5,double,int,FILE *ff_svod,FILE *ff_xml,GtkWidget *wpredok);

extern class iceb_rnfxml_data rek_zag_nn;
extern SQL_baza bd;

int rnnrd5w_5(short dn,short mn,short gn,short dk,short mk,short gk,
...
/*******************
if(iceb_openxml(dn,mn,gn,dk,mk,gk,imaf_xml,"J02","151","05",1,&period_type,&rek_zag_nn,&ff_xml,wpredok) != 0)
 return(1);

iceb_rnn_sap_xml(mn,gn,mk,"J02","151",5,1,"J0215105.xsd",period_type,&rek_zag_nn,ff_xml);
***********************/
...
if(proc_dot != 0.)
 {
/*************
  if(iceb_openxml(dn,mn,gn,dk,mk,gk,imaf_xml_dot,"J02","851","06",1,&period_type,&rek_zag_nn,&ff_xml_dot,wpredok) != 0)
   return(1);

  iceb_rnn_sap_xml(mn,gn,mk,"J02","851",6,1,"J0285106.xsd",period_type,&rek_zag_nn,ff_xml_dot);
*********************/
...
//  rnnrd5_start(gk,kvrt,mes,1,ff_svod_dot);
  ...
  for(int ii=0; ii < dod5->innn_per2.kolih(); ii++)
   {
    ...
    if(in2.ravno_atoi() == 0 || iceb_u_SRAV(in2.ravno(),"400000000000",0) == 0)  /*Значит нет индивидуального налогового номера*/
     {
...
for(int kkk=0; kkk < dod5->innn2d.kolih(); kkk++)
 {
  for(int ii=0; ii < dod5->innn_per2d.kolih(); ii++)
   {
    iceb_u_polen(dod5->innn_per2d.ravno(ii),&in2,1,'|');  
    if(iceb_u_SRAV(dod5->innn2d.ravno(kkk),in2.ravno(),0) != 0)
     continue;
...
//      inh2+=dod5->suma_nds2d.ravno(ii);
    ...
    iceb_u_polen(dod5->innn_per2d.ravno(ii),&period,2,'|');  
    iceb_u_rsdat1(&m,&g,period.ravno());
...
for(int kkk=0; kkk < dod5->innn2d1.kolih(); kkk++)
 {
  for(int ii=0; ii < dod5->innn_per2d1.kolih(); ii++)
   {
    iceb_u_polen(dod5->innn_per2d1.ravno(ii),&in2,1,'|');  
    if(iceb_u_SRAV(dod5->innn2d1.ravno(kkk),in2.ravno(),0) != 0)
     continue;
...
//      inh2+=dod5->suma_nds2d1.ravno(ii);
    ...
    iceb_u_polen(dod5->innn_per2d1.ravno(ii),&period,2,'|');  
    iceb_u_rsdat1(&m,&g,period.ravno());
...
rnnrd5_endras(dod5,proc_dot,0,ff_svod_naim,ff_xml,wpredok);
