/*$Id: l_zarsocf_v.c,v 1.20 2013/08/25 08:26:47 sasa Exp $*/
/*03.03.2016	23.06.2006	Белых А.И.	l_zarsocf_v.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarsocf_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_zarsocf_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_zarsocf_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(l_zarsocf_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(l_zarsocf_v_vvod),&data);
...
g_signal_connect(data.entry[E_PROC], "activate",G_CALLBACK(l_zarsocf_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_PROC]),data.rk.proc.ravno());
...
data.entry[E_KOD_N_VRAS] = gtk_entry_new ();
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_KOD_N_VRAS]),254);
...
g_signal_connect(data.entry[E_KOD_N_VRAS], "activate",G_CALLBACK(l_zarsocf_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KOD_N_VRAS]),data.rk.kod_n_vras.ravno());
gtk_widget_set_name(data.entry[E_KOD_N_VRAS],iceb_u_inttochar(E_KOD_N_VRAS));
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_zarsocf_v_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F10 %s",gettext("Выход"));
data.knopka[FK10]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK10],gettext("Завершение работы с меню без записи введенной информации"));
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_zarsocf_v_knopka),&data);
...
switch (enter)
 {
  ...
  case E_NAIM:
    ...
    break;
  case E_PROC:
    data->rk.proc.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
  case E_KOD_N_VRAS:
    data->rk.kod_n_vras.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
