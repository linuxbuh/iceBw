/*$Id: uosvhd2.c,v 1.21 2013/09/26 09:46:57 sasa Exp $*/
/*13.07.2015	29.10.2007	Белых А.И.	uosvhd2.c
Ввод шапки документа для расхода (операция внешняя)
...
gboolean   uosvhd2_m_v_key_press(GtkWidget *widget,GdkEventKey *event,class uosvhd2_m_data *data);
void    uosvhd2_m_v_vvod(GtkWidget *widget,class uosvhd2_m_data *data);
void  uosvhd2_m_v_knopka(GtkWidget *widget,class uosvhd2_m_data *data);
void   uosvhd2_m_rekviz(class uosvhd2_m_data *data);
void  uosvhd2_v_e_knopka(GtkWidget *widget,class uosvhd2_m_data *data);


extern SQL_baza bd;

int uosvhd2(class iceb_u_str *datadok,
...
datop,nds,pn,vd  from Uosdok where datd='%s' and nomd='%s'",data.rk.datadok.ravno_sqldata(),
  ...
  if(atoi(row[0]) != 2)
   {
    iceb_menu_soob("Это не расходный документ !",wpredok);
    return(1);
   }
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(uosvhd2_m_v_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATADOK],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATADOK], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_NOMDOK],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_NOMDOK], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_MAT_OT],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_MAT_OT], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KONTR],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KONTR], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.entry[E_OSNOV], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.entry[E_DOVER], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.entry[E_SHEREZ], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATA_VD],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATA_VD], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
gtk_box_pack_start (GTK_BOX (hbox[E_NOMNALNAK]), data.knopka_enter[E_NOMNALNAK], FALSE, FALSE,1);
g_signal_connect(data.knopka_enter[E_NOMNALNAK],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_NOMNALNAK],iceb_u_inttochar(E_NOMNALNAK));
gtk_widget_set_tooltip_text(data.knopka_enter[E_NOMNALNAK],gettext("Выбор даты"));
...
g_signal_connect(data.entry[E_NOMNALNAK], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATA_VNN],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATA_VNN], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATA_OPL],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATA_OPL], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_FOROP],"clicked",G_CALLBACK(uosvhd2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_FOROP], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_VID_DOG]),59);
gtk_box_pack_start (GTK_BOX (hbox[E_VID_DOG]), data.entry[E_VID_DOG], TRUE, TRUE,1);
g_signal_connect(data.entry[E_VID_DOG], "activate",G_CALLBACK(uosvhd2_m_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(uosvhd2_m_v_knopka),&data);
...
g_signal_connect(data.knopka[FK3],"clicked",G_CALLBACK(uosvhd2_m_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(uosvhd2_m_v_knopka),&data);
...
/******************************/
void uosvhd2_od(class uosvhd2_m_data *data)
{
char strsql[512];
...
switch (knop)
 {
  ...
  case E_NOMNALNAK:
     ...
     iceb_u_rsdat1(&m,&g,data->rk.datadok.ravno());
     if(m == 0)
      {
       iceb_menu_soob(gettext("Не введена дата документа !"),data->window);
       return;
      }
     data->rk.nomnalnak.new_plus(iceb_nomnalnak(m,g,data->window));
     gtk_entry_set_text(GTK_ENTRY(data->entry[E_NOMNALNAK]),data->rk.nomnalnak.ravno());
  ...
  case E_NOMDOK:
    ...
    uosgetnd(data->rk.datadok.ravno_god(),2,&data->rk.nomdok,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_NOMDOK]),data->rk.nomdok.ravno());
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("uosvhd2_m_v_knopka knop=%d\n",knop);
class iceb_u_str kod("");
class iceb_u_str naim("");
    ...
    if(data->rk.tipz == 1)
     {
      ...
      int vido1=0;
      int vido2=0;
      sprintf(strsql,"select vido from Uosras where kod='%s'",data->rk.kod_op.ravno());
      if(iceb_sql_readkey(strsql,&row,&cur,data->window) == 1)
       vido1=atoi(row[0]);

      if(l_uosopr(1,&kod,&naim,data->window) == 0)
       {
        sprintf(strsql,"select vido from Uosras where kod='%s'",kod.ravno());
        if(iceb_sql_readkey(strsql,&row,&cur,data->window) == 1)
         vido2=atoi(row[0]);
...
//g_print("uosvhd2_m_v_vvod enter=%d\n",enter);
switch (enter)
 {
