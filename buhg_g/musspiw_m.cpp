/*$Id:$*/
/*26.02.2017	26.02.2017	Белых А.И.	musspiw_m.c
  ...
  void read_rek()
   {
    ...
    rk->shetz.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_SHET])));
    rk->grup_mat.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_GRUPM])));
...
void  musspiw_radio02(GtkWidget *widget,class musspiw_data *data);
void  musspiw_radio12(GtkWidget *widget,class musspiw_data *data);
void  musspiw_radio22(GtkWidget *widget,class musspiw_data *data);

gboolean   musspiw_v_key_press(GtkWidget *widget,GdkEventKey *event,class musspiw_data *data);
void    musspiw_v_vvod(GtkWidget *widget,class musspiw_data *data);
void  musspiw_v_knopka(GtkWidget *widget,class musspiw_data *data);
void  musspiw_v_e_knopka(GtkWidget *widget,class musspiw_data *data);
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(musspiw_v_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(musspiw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(musspiw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SKLAD],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SKLAD], "activate",G_CALLBACK(musspiw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_GRUPM],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_GRUPM], "activate",G_CALLBACK(musspiw_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_GRUPM]),data.rk->grup_mat.ravno());
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(musspiw_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_SHET]),data.rk->shetz.ravno());
...
g_signal_connect(data.knopka_enter[E_KODMAT],"clicked",G_CALLBACK(musspiw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KODMAT], "activate",G_CALLBACK(musspiw_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(musspiw_v_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(musspiw_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(musspiw_v_knopka),&data);
...
switch (knop)
 {
  ...
  case E_GRUPM:
    if(iceb_l_grupmat(1,&kod,&naim,data->window) == 0)
      data->rk->grup_mat.z_plus(kod.ravno());
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_GRUPM]),data->rk->grup_mat.ravno());
    return;
  ...
  case E_SHET:
    iceb_vibrek(0,"Plansh",&data->rk->shetz,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_SHET]),data->rk->shetz.ravno());
    return;
 }
}
...
//g_print("musspiw_v_vvod enter=%d\n",enter);
...
switch (enter)
 {
  ...
  case E_SHET:
    data->rk->shetz.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
