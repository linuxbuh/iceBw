/*$Id: l_xrnngpn.c,v 1.17 2013/08/25 08:26:44 sasa Exp $*/
/*08.05.2015	14.04.2008	Белых А.И.	l_xrnngpn.c
Работа со списком групп полученных налоговых накладных
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_xrnngpn.h"
...
gboolean   l_xrnngpn_key_press(GtkWidget *widget,GdkEventKey *event,class l_xrnngpn_data *data);
void l_xrnngpn_vibor(GtkTreeSelection *selection,class l_xrnngpn_data *data);
void l_xrnngpn_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_xrnngpn_data *data);
void  l_xrnngpn_knopka(GtkWidget *widget,class l_xrnngpn_data *data);
void l_xrnngpn_add_columns (GtkTreeView *treeview);
void l_xrnngpn_udzap(class l_xrnngpn_data *data);
int  l_xrnngpn_prov_row(SQL_str row,class l_xrnngpn_data *data);
void l_xrnngpn_rasp(class l_xrnngpn_data *data);
int  l_xrnngpn_pvu(int,const char *kod,GtkWidget *wpredok);
void l_xrnngpn_create_list (class l_xrnngpn_data *data);

int l_xrnngpn_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_xrnngpn_p(class l_xrnngpn_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_xrnngpn_data *data);


extern SQL_baza  bd;

int   l_xrnngpn(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список групп для полученных накладных"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_xrnngpn_key_press),&data);
...
data.label_kolstr=gtk_label_new (gettext("Список групп для полученных накладных"));
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_xrnngpn_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_xrnngpn_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_xrnngpn_vibor),data);
...
sprintf(strsql,"select * from Reegrup1 order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_xrnngpn_prov_row(row,data) != 0)
    continue;
...
l_xrnngpn_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Список групп полученных налоговых накладных"));
...
//g_print("l_xrnngpn_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_xrnngpn_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_xrnngpn_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_xrnngpn_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_xrnngpn_create_list(data);
...
if(l_xrnngpn_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Reegrup1 where kod='%s'",
data->kodv.ravno());
...
sprintf(strsql,"gpn%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Список групп полученных налоговых накладных"));
...
iceb_zagolov(gettext("Список групп полученных налоговых накладных"),0,0,0,0,0,0,ff,data->window);
...
int  l_xrnngpn_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select gr from Reenn1 where gr='%s' limit 1",kod);
...
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
    iceb_u_str soob;
    sprintf(strsql,"%s Reenn1",gettext("Удалить невозможно ! Есть записи в таблице"));
    soob.plus(strsql);
...
char strsql[512];
sprintf(strsql,"select kod from Reegrup1");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_xrnngpn_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Reegrup1 where kod='%s'",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
