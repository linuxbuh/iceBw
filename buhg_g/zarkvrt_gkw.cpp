/*$Id: zarkvrt_gkw.c,v 1.4 2013/08/13 06:10:03 sasa Exp $*/
/*13.07.2015	21.04.2005	Белых А.И.	zarkvrt_gkw.c
...
char strsql[1024];
char bros[512];
class iceb_u_str sheta_doh("");
class iceb_u_str sheta_pnal("");

if(iceb_u_polen(shkas_shkom,&sheta_doh,1,'>') != 0)
  sheta_doh.new_plus(shkas_shkom);
else
  iceb_u_polen(shkas_shkom,&sheta_pnal,2,'>');
  
int kolpol=iceb_u_pole2(sheta_doh.ravno(),';');
...
for(int nn=0; nn < kolpol ; nn++)
 {
  ...
  iceb_u_polen(sheta_doh.ravno(),strsql,sizeof(strsql),nn+1,';');
  if(strsql[0] == '\0')
   continue;
...
if(sheta_pnal.ravno()[0] != '\0')
 {
  kolpol=iceb_u_pole2(sheta_pnal.ravno(),';');
  //Смотрим подоходный налог начисленный и выплаченный
  for(int nn=0; nn < kolpol ; nn++)
   {

    iceb_u_polen(sheta_pnal.ravno(),strsql,sizeof(strsql),nn+1,';');
    if(strsql[0] == '\0')
     continue;
...
for(int nom=0; nom < kod_kom.kolih(); nom++)
 {
  ...
  if(iceb_u_strstrm(kod.ravno(),prist_k_tabnom) == 1)
   {
    sprintf(strsql,"select fio,inn from Kartb where tabn=%s",iceb_u_adrsimv(iceb_u_strlen(prist_k_tabnom),kod.ravno()));
  ...
  if(iceb_u_strlen(inn.ravno()) < 10)
   {
    sprintf(strsql,"%s %s %s!",gettext("Идентификационный номер имеет меньше 10 знаков"),inn.ravno(),kod.ravno());
    iceb_menu_soob(strsql,wpredok);
   }   

  zarkvrt_zapvtablw(imatmptab,inn.ravno(),kod_doh,"",suma_n,suma_v,podoh_n,podoh_v,"0000-00-00","0000-00-00",fio.ravno_filtr(),kod.ravno(),ff_prot,wpredok);
