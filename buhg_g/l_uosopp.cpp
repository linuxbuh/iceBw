/*$Id: l_uosopp.c,v 1.28 2013/08/25 08:26:41 sasa Exp $*/
/*08.05.2015	17.07.2005	Белых А.И.	l_uosopp.c
...
gboolean   l_uosoppkey_press(GtkWidget *widget,GdkEventKey *event,class l_uosoppdata *data);
void l_uosoppvibor(GtkTreeSelection *selection,class l_uosoppdata *data);
void l_uosoppv_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,class l_uosoppdata *data);
void  l_uosoppknopka(GtkWidget *widget,class l_uosoppdata *data);
void l_uosoppadd_columns (GtkTreeView *treeview);
void l_uosoppcreate_list(class l_uosoppdata *data);
int  l_uosoppprov_row(SQL_str row,class l_uosoppdata *data);

int l_uosopp_v(iceb_u_str *kodzap,GtkWidget *wpredok);
int l_uosopp_p(class uslop_rek *datap,GtkWidget *wpredok);
int  l_uosopppvu(int met,const char *kod,GtkWidget *wpredok);
void l_uosoppudzap(class l_uosoppdata *data);
void l_uosoppunk(GtkWidget *wpredok);
void l_uosopprasp(class l_uosoppdata *data);
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosoppkey_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uosoppknopka),&data);
...
switch(event->keyval)
 {
  ...
  case ICEB_REG_L:
  case ICEB_REG_R:
    printf("l_uosoppkey_press-Нажата клавиша Shift\n");
...
//printf("l_uosoppvibor-%s %d\n",data->kodv.ravno(),data->snanomer);

}
/****************************/
/*Выбор строки по двойному клику мышкой*/
/**********************/
void l_uosoppv_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uosoppdata *data)
{
...
if(l_uosopppvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uospri where kod='%s'",
data->kodv.ravno());
...
sprintf(strsql,"select kodop from Uosdok where kodop='%s' and tipz=1 limit 1",kod);
...
sprintf(strsql,"select ko from Reenn1 where ko='%s' and mi=3 limit 1",kod);
...
SQLCURSOR cur;
int kolstr;
sprintf(strsql,"select kod from Uospri");
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uosopppvu(0,row[0],wpredok) != 0)
    continue;
  sprintf(strsql,"delete from Uospri where kod='%s'",row[0]);
...
sprintf(strsql,"uosopp%d.lst",getpid());
...
fprintf(ff,"%s:%s\n",gettext("Подсистема"),gettext("Учёт основных средств"));
fprintf(ff,"\
----------------------------------------------------------------------------------------------\n");
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(row[4][0] == '0')
    vid_op.new_plus(gettext("Внешняя"));
  if(row[4][0] == '1')
    vid_op.new_plus(gettext("Внутренняя"));
  if(row[4][0] == '2')
    vid_op.new_plus(gettext("Смена стоимости"));

  //Проводки
  if(row[5][0] == '0')
    prov.new_plus(gettext("Да"));
  if(row[5][0] == '1')
    prov.new_plus(gettext("Нет"));

  fprintf(ff,"%-*s %-*.*s %-*.*s %-*s %s %s\n",
  iceb_u_kolbait(4,row[0]),
  row[0],
  iceb_u_kolbait(30,row[1]),iceb_u_kolbait(30,row[1]),row[1],
  iceb_u_kolbait(12,vid_op.ravno()),
  iceb_u_kolbait(12,vid_op.ravno()),
  vid_op.ravno(),
