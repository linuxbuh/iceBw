/*$Id: akt_sv.c,v 5.6 2013/05/17 14:56:27 sasa Exp $*/
/*05.11.2009	26.01.2007	Белых А.И.	iceb_akt_sv.c
Распечатка информации по акту сверки
*/
#include "iceb_libbuh.h"

void akt_sv_it(double deb_start,double kre_start,double deb_oborot,double kre_oborot,double deb_ob_mes,double kre_ob_mes,FILE *ff);


void iceb_akt_sv(class akt_sverki *akt_svr,FILE *ff, /*с месячными итогами*/
FILE *ff_bi) /*Без месячных итогов*/
{

iceb_zagsh(akt_svr->shet.ravno(),ff,NULL);
iceb_zagsh(akt_svr->shet.ravno(),ff_bi,NULL);
...
for(int ii=0; ii < akt_svr->data_nomd.kolih(); ii++)
 {
  ...
  if(mz != m || gz != g)
   {
    if(mz != 0)
       akt_sv_it(akt_svr->start_saldo_deb,akt_svr->start_saldo_kre,deb_oborot,kre_oborot,deb_ob_mes,kre_ob_mes,ff);
...
akt_sv_it(akt_svr->start_saldo_deb,akt_svr->start_saldo_kre,deb_oborot,kre_oborot,deb_ob_mes,kre_ob_mes,ff);
akt_sv_it(akt_svr->start_saldo_deb,akt_svr->start_saldo_kre,deb_oborot,kre_oborot,deb_ob_mes,kre_ob_mes,ff_bi);
...
void akt_sv_it(double deb_start,double kre_start,
