/*$Id: iceb_l_opmup_p.c,v 1.15 2013/08/25 08:27:06 sasa Exp $*/
/*02.12.2016	05.05.2004	Белых А.И.	iceb_l_opmup_p.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(opmup_p_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(opmup_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(opmup_p_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(opmup_p_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F4 %s",gettext("Очистить"));
data.knopka[FK4]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK4],gettext("Очистить меню от введеноой информации"));
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(opmup_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(opmup_p_knopka),&data);
