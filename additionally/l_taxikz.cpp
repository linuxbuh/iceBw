/*$Id: l_taxikz.c,v 1.9 2013/08/13 06:10:24 sasa Exp $*/
/*12.05.2015	30.05.2009	Белых А.И.	l_taxikz.c
Работа со списком кодов завершения
*/
#include        <stdlib.h>
#include        <errno.h>
#include        <time.h>
...
gboolean   l_taxikz_key_press(GtkWidget *widget,GdkEventKey *event,class l_taxikz_data *data);
void l_taxikz_vibor(GtkTreeSelection *selection,class l_taxikz_data *data);
void l_taxikz_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_taxikz_data *data);
void  l_taxikz_knopka(GtkWidget *widget,class l_taxikz_data *data);
void l_taxikz_add_columns (GtkTreeView *treeview);
void l_taxikz_udzap(class l_taxikz_data *data);
int  l_taxikz_prov_row(SQL_str row,class l_taxikz_data *data);
void l_taxikz_rasp(class l_taxikz_data *data);
int  l_taxikz_pvu(int,const char *kod,GtkWidget *wpredok);
void l_taxikz_create_list (class l_taxikz_data *data);

int l_taxikz_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_taxikz_p(class l_taxikz_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_taxikz_data *data);
...
int   l_taxikz(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",name_system,"Список кодов завершения");
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_taxikz_key_press),&data);
...
data.label_kolstr=gtk_label_new ("Список кодов завершения");
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_taxikz_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_taxikz_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_taxikz_vibor),data);
...
sprintf(strsql,"select * from Taxikzz order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_taxikz_prov_row(row,data) != 0)
    continue;
...
l_taxikz_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus("Список кодов завершения");
...
//g_print("l_taxikz_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    if(l_taxikz_v(&data->kod_kat_tv,data->window) == 0)
      l_taxikz_create_list(data);
    return;  
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    if(l_taxikz_v(&data->kodv,data->window) == 0)
      l_taxikz_create_list(data);
    return;  
  ...
  case FK4:
    if(l_taxikz_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_taxikz_create_list(data);
...
class l_taxikz_data *data)
{
//printf("l_taxikz_v_row\n");
//printf("l_taxikz_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  g_signal_emit_by_name(data->knopka[SFK2],"clicked");
  return;
...
if(l_taxikz_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[300];

sprintf(strsql,"delete from Taxikzz where kod=%s",data->kodv.ravno());
...
imaf.plus(strsql);
naimot.plus("Список кодов завершения");
...
iceb_u_zagolov("Список кодов завершения",0,0,0,0,0,0,iceb_get_pnk("00",0,data->window),ff);
...
int  l_taxikz_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[300];

sprintf(strsql,"select kz from Taxizak where kz=%s limit 1",kod);
...
char strsql[500];
sprintf(strsql,"select kod from Taxikzz");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_taxikz_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Taxikzz where kod=%s",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
