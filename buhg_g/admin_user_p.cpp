/*$Id: admin_user_p.c,v 1.8 2013/08/25 08:26:31 sasa Exp $*/
/*10.07.2015	14.11.2010	Белых А.И.	admin_user_p.c
...
class admin_user_p_data
 {
  ...
  void read_rek()
   {
    rk->login.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_LOGIN])));
    rk->imabaz.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_IMABAZ])));
    rk->host.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_HOST])));
   }
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(admin_user_p_key_press),&data);
...
g_signal_connect(data.entry[E_LOGIN], "activate",G_CALLBACK(admin_user_p_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_LOGIN]),data.rk->login.ravno());
...
g_signal_connect(data.entry[E_IMABAZ], "activate",G_CALLBACK(admin_user_p_vvod),&data);
...
g_signal_connect(data.entry[E_HOST], "activate",G_CALLBACK(admin_user_p_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_HOST]),data.rk->host.ravno());
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(admin_user_p_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(admin_user_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(admin_user_p_knopka),&data);
...
switch (enter)
 {
  case E_LOGIN:
    data->rk->login.new_plus(gtk_entry_get_text(GTK_ENTRY(data->entry[E_LOGIN])));
    break;
  case E_IMABAZ:
    data->rk->imabaz.new_plus(gtk_entry_get_text(GTK_ENTRY(data->entry[E_IMABAZ])));
    break;
  case E_HOST:
    data->rk->host.new_plus(gtk_entry_get_text(GTK_ENTRY(data->entry[E_HOST])));
    break;
 }
