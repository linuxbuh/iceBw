/*$Id: l_xrnnv_v.c,v 1.17 2013/08/25 08:26:45 sasa Exp $*/
/*02.03.2016	07.05.2008	Белых А.И.	l_xrnnv_v.c
  ...
  void read_rek()
   {
    ...
    datv.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_DATV])));
   }
  void clear()
   {
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_xrnnv_v_key_press),&data);
...
g_signal_connect(data.entry[E_NOM_NAL_NAK], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAVNN],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAVNN], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.entry[E_SUMD1], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.entry[E_NDS1], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.entry[E_INN], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_NAIM_KONTR],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_NAIM_KONTR], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOMENT], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_GR],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_GR], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_OP],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_OP], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_VIDDOK],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_VIDDOK], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
...
data.knopka_enter[E_DATV]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_DATV]), data.knopka_enter[E_DATV], FALSE, FALSE,1);
g_signal_connect(data.knopka_enter[E_DATV],"clicked",G_CALLBACK(l_xrnnv_v_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_DATV],iceb_u_inttochar(E_DATV));
gtk_widget_set_tooltip_text(data.knopka_enter[E_DATV],gettext("Выбор даты"));

data.entry[E_DATV] = gtk_entry_new ();
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_DATV]),10);
gtk_box_pack_start (GTK_BOX (hbox[E_DATV]), data.entry[E_DATV], TRUE, TRUE,1);
g_signal_connect(data.entry[E_DATV], "activate",G_CALLBACK(l_xrnnv_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_DATV]),data.datv.ravno());
gtk_widget_set_name(data.entry[E_DATV],iceb_u_inttochar(E_DATV));
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_xrnnv_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_xrnnv_v_knopka),&data);
...
switch (enter)
 {
  case E_NOM_NAL_NAK:
    data->nom_nal_nak.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
  case E_DATAVNN:
    data->datavnn.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
