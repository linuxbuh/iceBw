/*$Id: zar_vedpz_m.c,v 1.13 2013/08/25 08:27:01 sasa Exp $*/
/*05.12.2014	08.12.2006	Белых А.И.	zar_vedpz_m.c
Меню для распечатки карточки с начислениями и удержиниями за период
*/
#include "buhg_g.h"
#include "zar_vedpz.h"
enum
  ...
  void read_rek()
   {
    ...
    rk->kod_grup.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_PODR])));
    rk->shet.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_PODR])));
...
gboolean   zar_vedpz_m_key_press(GtkWidget *widget,GdkEventKey *event,class zar_vedpz_m_data *data);
void  zar_vedpz_m_knopka(GtkWidget *widget,class zar_vedpz_m_data *data);
void    zar_vedpz_m_vvod(GtkWidget *widget,class zar_vedpz_m_data *data);


void  zar_vedpz_m_e_knopka(GtkWidget *widget,class zar_vedpz_m_data *data);
...
sprintf(strsql,"%s %s",iceb_get_namesystem(),gettext("Распечатать ведомость производственных затрат"));
label=gtk_label_new(gettext("Распечатать ведомость производственных затрат"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_vedpz_m_key_press),&data);
...
data.radiobutton2[0]=gtk_radio_button_new_with_label_from_widget(NULL,gettext("Начисления и удержания"));
data.radiobutton2[1]=gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(data.radiobutton2[0]),gettext("Толькo начисления"));
data.radiobutton2[2]=gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(data.radiobutton2[1]),gettext("Только удержания"));
...
gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data.radiobutton2[data.rk->metka_pr_ras]),TRUE); //Устанавливем активной кнопку
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(zar_vedpz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(zar_vedpz_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(zar_vedpz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(zar_vedpz_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(zar_vedpz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(zar_vedpz_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KODGR],"clicked",G_CALLBACK(zar_vedpz_m_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_KODGR],iceb_u_inttochar(E_KODGR));
gtk_widget_set_tooltip_text(data.knopka_enter[E_KODGR],gettext("Выбор группы подразделения"));

data.entry[E_KODGR] = gtk_entry_new();
gtk_box_pack_start (GTK_BOX (hbox[E_KODGR]), data.entry[E_KODGR], TRUE, TRUE, 0);
g_signal_connect(data.entry[E_KODGR], "activate",G_CALLBACK(zar_vedpz_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KODGR]),data.rk->kod_grup.ravno());
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(zar_vedpz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(zar_vedpz_m_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zar_vedpz_m_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(zar_vedpz_m_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zar_vedpz_m_knopka),&data);
...
switch (knop)
 {
  ...
  case E_KODGR:
    if(l_gruppod(1,&kod,&naim,data->window) == 0)
     data->rk->kod_grup.z_plus(kod.ravno());
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KODGR]),data->rk->kod_grup.ravno());
    return;
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zar_vedpz_m_knopka knop=%d\n",knop);
...
//g_print("zar_vedpz_m_vvod enter=%d\n",enter);
...
switch (enter)
 {
  ...
  case E_PODR:
    ...
    break;
  case E_KODGR:
    data->rk->kod_grup.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
