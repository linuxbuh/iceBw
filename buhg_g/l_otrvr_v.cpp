/*$Id: l_otrvr_v.c,v 1.20 2014/02/28 05:20:59 sasa Exp $*/
/*30.11.2016	08.09.2006	Белых А.И.	l_otrvr_v.c
...
class l_otrvr_v_data
 {
  ...
  //Реквизиты меню
  class iceb_u_str tabnom_v;
  class iceb_u_str kod_v;
  ...
  void read_rek()
   {
    if(tabnom.getdlinna() <= 1)
     {
      tabnom_v.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_TABNOM])));
      kod_v.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_KOD_TAB])));
     }
...
gboolean   l_otrvr_v_v_key_press(GtkWidget *widget,GdkEventKey *event,class l_otrvr_v_data *data);
void    l_otrvr_v_v_vvod(GtkWidget *widget,class l_otrvr_v_data *data);
void  l_otrvr_v_v_knopka(GtkWidget *widget,class l_otrvr_v_data *data);
void otrvr_t_kkvt(char *skod,char *nkod,GtkWidget *wpredok);
int otrvr_t_zap(class l_otrvr_v_data *data);
int otrvr_t_pk(char *kod,GtkWidget *wpredok);
void  otrvr_t_v_e_knopka(GtkWidget *widget,class l_otrvr_v_data *data);

int l_otrvr_v_zap(class l_otrvr_v_data *data);
...
class l_otrvr_v_data data;

data.tabnom.new_plus(tabnom->ravno());
data.tabnom_v.new_plus(tabnom->ravno());

data.mp=mp;
data.gp=gp;

data.kodk.new_plus(vid_tab->ravno());
data.kod_v.new_plus(vid_tab->ravno());
...
data.oklad=okladw(data.tabnom.ravno_atoi(),&data.metka_sp,wpredok);
...
if(data.tabnom.getdlinna() > 1)
 { 
  ...
  sprintf(strsql,"select naik from Tabel where kod=%d",data.kodk.ravno_atoi());
  if(iceb_sql_readkey(strsql,&row,&cur,wpredok) == 1)
   naim_tab.new_plus(row[0]);


  sprintf(strsql,"select * from Ztab where tabn=%d and god=%d and mes=%d and kodt=%d and nomz=%d",
  data.tabnom.ravno_atoi(),data.gp,data.mp,data.kodk.ravno_atoi(),data.nom_zapk);
  if(iceb_sql_readkey(strsql,&row,&cur,wpredok) != 1)
   {
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_otrvr_v_v_key_press),&data);
...
if(data.tabnom.getdlinna() <= 1)
...
else
 {
  ...
  zagolov.ps_plus(data.tabnom.ravno());
  zagolov.plus(" ");
  zagolov.plus(fio.ravno());
  
  zagolov.ps_plus(data.kodk.ravno());
  zagolov.plus(" ");
...
if(data.tabnom.getdlinna() <= 1)
 {
  ...
  g_signal_connect(data.entry[E_TABNOM], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
  sprintf(strsql,"%s %s",data.tabnom_v.ravno(),fio.ravno());
  gtk_entry_set_text(GTK_ENTRY(data.entry[E_TABNOM]),strsql);
  ...
  g_signal_connect(data.entry[E_KOD_TAB], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
  sprintf(strsql,"%s %s",data.kod_v.ravno(),naim_tab.ravno());
  gtk_entry_set_text(GTK_ENTRY(data.entry[E_KOD_TAB]),strsql);
...
g_signal_connect(data.entry[E_KOL_OTR_DNEI], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOL_OTR_HAS], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOL_KAL_DNEI], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOL_RAB_DNEI], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOL_RAB_HAS], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_KOMENT], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_NOM_ZAP], "activate",G_CALLBACK(l_otrvr_v_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_otrvr_v_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_otrvr_v_v_knopka),&data);
...
if(data.voz == 0)
 {
  ...
  vid_tab->new_plus(data.kod_v.ravno());
 }
return(data.voz);
...
void  otrvr_t_v_e_knopka(GtkWidget *widget,class l_otrvr_v_data *data)
{
class iceb_u_str kod(0);
class iceb_u_str naim(0);
...
switch (knop)
 {
  ...
  case E_TABNOM:
    if(l_sptbn(1,&kod,&naim,0,data->window) == 0)
     {
      data->tabnom_v.new_plus(kod.ravno());
      data->tabnom_v.plus(" ");
      data->tabnom_v.plus(naim.ravno());
     }
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_TABNOM]),data->tabnom_v.ravno());
    return;  

  case E_KOD_TAB:
    if(l_zarvidtab(1,&kod,&naim,data->window) == 0)
     {
      data->kod_v.new_plus(kod.ravno());
      data->kod_v.plus(" ");
      data->kod_v.plus(naim.ravno());
     }
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KOD_TAB]),data->kod_v.ravno());
    return;  
 }
}
...
//g_print("l_otrvr_v_v_vvod enter=%d\n",enter);
...
switch (enter)
 {
  ...
  case E_TABNOM:
    data->tabnom_v.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    
    //Читаем фамилию
    sprintf(strsql,"select tabn,fio from Kartb where tabn=%d",data->tabnom_v.ravno_atoi());
    if(iceb_sql_readkey(strsql,&row,&cur,data->window) == 1)
     {
      data->tabnom_v.new_plus(row[0]);
      data->tabnom_v.plus(" ");
      data->tabnom_v.plus(row[1]);
     }
  ...
  case E_KOD_TAB:
    data->kod_v.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));

    //Читаем наименование табеля
    sprintf(strsql,"select kod,naik from Tabel where kod=%d",data->kod_v.ravno_atoi());
    if(iceb_sql_readkey(strsql,&row,&cur,data->window) == 1)
     {
      data->kod_v.new_plus(row[0]);
      data->kod_v.plus(" ");
      data->kod_v.plus(row[1]);
     }
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KOD_TAB]),data->kod_v.ravno());
    break;
...
int voz=zaptabelw(metka_rr,data->mp,data->gp,data->tabnom_v.ravno_atoi(),data->kod_v.ravno_atoi(),
