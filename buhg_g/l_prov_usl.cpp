/*$Id: l_prov_usl.c,v 1.41 2013/12/31 11:49:14 sasa Exp $*/
/*24.05.2016	03.11.2005	Белых А.И.	l_prov_usl.c
...
class prov_usl_data
 {
  ...
  double     suma_kre;  
  int kekv;
  short metka_avp; /*0-проводки автоматически не делались 1- делались*/
  ...
  prov_usl_data()
   {
    ...
    naim_podr.new_plus("");
    metka_avp=0;
    metkasys.plus(ICEB_MP_USLUGI);
   }

 };
gboolean   prov_usl_key_press(GtkWidget *widget,GdkEventKey *event,class prov_usl_data *data);
void prov_usl_vibor(GtkTreeSelection *selection,class prov_usl_data *data);
void prov_usl_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class prov_usl_data *data);
void  prov_usl_knopka(GtkWidget *widget,class prov_usl_data *data);
void prov_usl_add_columns (GtkTreeView *treeview);
void prov_usl_create_list (class prov_usl_data *data);

void       prov_usl_radio0(GtkWidget *,class prov_usl_data *);

void prov_usl_sapka_menu(class prov_usl_data *data);
int prov_usl_prpvblok(class prov_usl_data *data);
void avtprouslw(int tipz,short dd,short md,short gd,const char *kop,const char *nn,int podr,const char *kor,double sumkor,const char *nn1,float pnds,FILE *ff_prot,GtkWidget *wpredok);
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(prov_usl_key_press),&data);
...
g_signal_connect(data.radiobutton[0], "clicked",G_CALLBACK(prov_usl_radio0),&data);
...
g_signal_connect(data.radiobutton[1], "clicked",G_CALLBACK(prov_usl_radio0),&data);
...
g_signal_connect(data.radiobutton[2], "clicked",G_CALLBACK(prov_usl_radio0),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[SFK4], "clicked",G_CALLBACK(prov_usl_knopka),&data);
gtk_box_pack_start(GTK_BOX(vbox1), data.knopka[SFK4],TRUE,TRUE, 0);
gtk_widget_set_tooltip_text(data.knopka[SFK4],gettext("Просмотр протокола хода автоматичного виконання проводок"));
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(prov_usl_knopka),&data);
gtk_widget_set_tooltip_text(data.knopka[FK5],gettext("Насторойка автоматического выполнения проводок"));
...
g_signal_connect(data.knopka[SFK5], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[FK6], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[FK9], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(prov_usl_knopka),&data);
...
printf("prov_usl_add_columns end\n");
...
switch (knop)
 {
  ...
  case FK2:
    if(prov_usl_prpvblok(data) != 0)
     return;
    if(iceb_vprov(data->metkasys.ravno(),"","","","","",data->nomdok.ravno(),data->kodop.ravno(),data->podr,
    data->dd,data->md,data->gd,data->kontr.ravno(),data->vrem_v,data->ktoz_v,6,data->tipz,0,0,data->window) != 0)
         return;  
  ...
  case SFK2:
    ...
    data->dd,data->md,data->gd,data->kontr.ravno(),data->vrem_v,data->ktoz_v,6,data->tipz,data->val_v,data->kekv,data->window) != 0)
         return;  
  ...
  case FK4:
    ...
    if(data->metka_nsnp == 1)
      avtprouslw(data->tipz,data->dd,data->md,data->gd,data->kodop.ravno(),
      data->nomdok.ravno(),data->podr,data->kontr.ravno(),data->sumkor,data->nomdokp.ravno(),data->pnds,ff_prot,data->window);
    else
    avtprousl1w(data->tipz,data->dd,data->md,data->gd,data->nomdok.ravno(),ff_prot,data->window);
  ...
  case SFK5:
    iceb_raspprov(data->zapros.ravno(),data->dd,data->md,data->gd,data->nomdok.ravno(),4,data->window);
    return;  
  ...
  case FK5:
    menuvnpw(2,data->tipz,data->window);
    return;  
  
  case FK9:
   
    l_opldok(1,data->dd,data->md,data->gd,data->nomdok.ravno(),data->podr,
    skopluslw(data->dd,data->md,data->gd,data->nomdok.ravno(),data->podr,data->window),
    data->window);
  ...
  case FK10:
    ...
    prosprusw(0,data->podr,data->dd,data->md,data->gd,data->nomdok.ravno(),data->tipz,\
    data->lnds,data->kodop.ravno(),data->pnds,NULL,data->window);
...
switch(event->keyval)
 {
  ...
  case ICEB_REG_L:
  case ICEB_REG_R:
    printf("prov_usl_key_press-Нажата клавиша Shift\n");
...
stroka.new_plus(gettext("Работа с проводками (учёт услуг)"));
...
void  prov_usl_radio0(GtkWidget *widget,class prov_usl_data *data)
{
