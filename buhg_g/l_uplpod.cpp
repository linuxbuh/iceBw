/*$Id: l_uplpod.c,v 1.17 2013/08/25 08:26:42 sasa Exp $*/
/*08.05.2015	03.03.2008	Белых А.И.	l_uplpod.c
Работа со списком подразделений в подсистеме "Учёт путевых листов"
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_uplpod.h"
...
gboolean   l_uplpod_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplpod_data *data);
void l_uplpod_vibor(GtkTreeSelection *selection,class l_uplpod_data *data);
void l_uplpod_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uplpod_data *data);
void  l_uplpod_knopka(GtkWidget *widget,class l_uplpod_data *data);
void l_uplpod_add_columns (GtkTreeView *treeview);
void l_uplpod_udzap(class l_uplpod_data *data);
int  l_uplpod_prov_row(SQL_str row,class l_uplpod_data *data);
void l_uplpod_rasp(class l_uplpod_data *data);
int  l_uplpod_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uplpod_create_list (class l_uplpod_data *data);

int l_uplpod_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uplpod_p(class l_uplpod_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uplpod_data *data);


extern SQL_baza  bd;

int   l_uplpod(int metka_rr, //0-ввод и корек. 1-выбор
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uplpod_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uplpod_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uplpod_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uplpod_vibor),data);
...
sprintf(strsql,"select * from Uplpodr order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uplpod_prov_row(row,data) != 0)
    continue;
...
l_uplpod_add_columns (GTK_TREE_VIEW (data->treeview));
...
//g_print("l_uplpod_knopka knop=%d\n",knop);
data->kl_shift=0;
switch (knop)
 {
  case FK2:
    l_uplpod_v(&data->kod_gr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uplpod_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uplpod_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uplpod_create_list(data);
...
if(l_uplpod_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uplpodr where kod=%d",
...
sprintf(strsql,"uplpod%d.lst",getpid());
...
int  l_uplpod_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kp from Upldok where kp=%d limit 1",atoi(kod));
...
sprintf(strsql,"select kp from Upldok1 where kp=%d limit 1",atoi(kod));
...
sprintf(strsql,"select kp from Upldok2a where kp=%d limit 1",atoi(kod));
...
sprintf(strsql,"select kp from Upldok3 where kp=%d limit 1",atoi(kod));
...
char strsql[512];
sprintf(strsql,"select kod from Uplpodr");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uplpod_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uplpodr where kod=%d",atoi(row[0]));
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
