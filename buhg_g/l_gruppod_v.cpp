/*$Id: l_gruppod_v.c,v 1.27 2013/08/25 08:26:35 sasa Exp $*/
/*29.02.2016	21.06.2006	Белых А.И.	l_gruppod_v.c
...
class l_gruppod_v_data
 {
  ...
  class l_gruppod_rek rk;
  class iceb_u_str kod_gr;  
  
  l_gruppod_v_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
gboolean   l_gruppod_v_key_press(GtkWidget *widget,GdkEventKey *event,class l_gruppod_v_data *data);
void  l_gruppod_v_knopka(GtkWidget *widget,class l_gruppod_v_data *data);
void    l_gruppod_v_vvod(GtkWidget *widget,class l_gruppod_v_data *data);
int l_gruppod_pk(const char *god,GtkWidget*);
int l_gruppod_zap(class l_gruppod_v_data *data);
void l_gruppod_kkvt(const char *kods,const char *kodn,GtkWidget *wpredok);
...
if(data.kod_gr.getdlinna() >  1)
 {
  ...
  sprintf(strsql,"select * from Zargrupp where kod='%s'",data.kod_gr.ravno());
  SQL_str row;
  SQLCURSOR cur;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_gruppod_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_gruppod_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_gruppod_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_gruppod_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_gruppod_v_knopka),&data);
...
if(iceb_u_SRAV(data->kod_gr.ravno(),data->rk.kod.ravno(),0) != 0)
  if(l_gruppod_pk(data->rk.kod.ravno(),data->window) != 0)
     return(1);

if(data->kod_gr.getdlinna() <= 1)
 {

  sprintf(strsql,"insert into Zargrupp \
values ('%s','%s',%d,%ld)",
...
else
 {
  ...
  sprintf(strsql,"update Zargrupp \
set \
kod='%s',\
naik='%s',\
ktoz=%d,\
...
if(metkakor == 1)
 l_gruppod_kkvt(data->kod_gr.ravno(),data->rk.kod.ravno(),data->window);
...
/**********************************/
int l_gruppod_pk(const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
void l_gruppod_kkvt(const char *skod,const char *nkod,GtkWidget *wpredok)
{

char strsql[512];

sprintf(strsql,"update Podr set grup='%s' where grup='%s'",nkod,skod);
