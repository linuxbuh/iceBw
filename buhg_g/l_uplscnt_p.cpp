/*$Id: l_uplscnt_p.c,v 1.10 2013/08/25 08:26:42 sasa Exp $*/
/*10.03.2008	10.03.2008	Белых А.И.	l_uplscnt_p.c
...
void l_uplscnt_p_clear(class l_uplscnt_p_data *data);
void    l_uplscnt_p_vvod(GtkWidget *widget,class l_uplscnt_p_data *data);
void  l_uplscnt_p_knopka(GtkWidget *widget,class l_uplscnt_p_data *data);
gboolean   l_uplscnt_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_uplscnt_p_data *data);


int l_uplscnt_p(class l_uplscnt_rek *rek_poi,GtkWidget *wpredok)
{
class l_uplscnt_p_data data;
char strsql[512];
data.rk=rek_poi;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uplscnt_p_key_press),&data);
...
GtkWidget *label=gtk_label_new(gettext("Дата"));
...
g_signal_connect(data.entry[E_DATA], "activate",G_CALLBACK(l_uplscnt_p_vvod),&data);
...
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_KOD_TOP]),100);
gtk_box_pack_start (GTK_BOX (hbox[E_KOD_TOP]), data.entry[E_KOD_TOP], TRUE, TRUE,1);
g_signal_connect(data.entry[E_KOD_TOP], "activate",G_CALLBACK(l_uplscnt_p_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_uplscnt_p_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(l_uplscnt_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_uplscnt_p_knopka),&data);
...
//g_print("l_uplscnt_p_knopka knop=%d\n",knop);
...
//g_print("l_uplscnt_p_vvod enter=%d\n",enter);
