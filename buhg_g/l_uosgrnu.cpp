/*$Id: l_uosgrnu.c,v 1.21 2013/12/31 11:49:15 sasa Exp $*/
/*08.05.2015	18.10.2007	Белых А.И.	l_uosgrnu.c
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список групп налогового учёта"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosgrnu_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uosgrnu_knopka),&data);
...
iceb_zagolov(gettext("Список групп налогового учёта"),0,0,0,0,0,0,ff,data->window);
...
class iceb_u_str alam("");
while(cur.read_cursor(&row) != 0)
 {
  if(atoi(row[5]) == 0)
   tipam.new_plus(gettext("Пообъектная"));
  else
   tipam.new_plus(gettext("В целом по группе"));

  if(atoi(row[6]) == 0)
   alam.new_plus(gettext("От начальной балансовой стоимости"));
  else
   alam.new_plus(gettext("От остаточной балансовой стоимости"));
  ...
  iceb_u_kolbait(20,alam.ravno()),iceb_u_kolbait(20,alam.ravno()),alam.ravno());
 }
fprintf(ff,"\
----------------------------------------------------------------------------------\n");
