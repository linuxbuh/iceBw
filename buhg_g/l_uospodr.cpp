/*$Id: l_uospodr.c,v 1.20 2013/08/25 08:26:41 sasa Exp $*/
/*08.05.2015	18.10.2007	Белых А.И.	l_uospodr.c
Работа со списком подразделений в учёте основных средств
*/
#include        <errno.h>
...
gboolean   l_uospodr_key_press(GtkWidget *widget,GdkEventKey *event,class l_uospodr_data *data);
void l_uospodr_vibor(GtkTreeSelection *selection,class l_uospodr_data *data);
void l_uospodr_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uospodr_data *data);
void  l_uospodr_knopka(GtkWidget *widget,class l_uospodr_data *data);
void l_uospodr_add_columns (GtkTreeView *treeview);
void l_uospodr_udzap(class l_uospodr_data *data);
int  l_uospodr_prov_row(SQL_str row,class l_uospodr_data *data);
void l_uospodr_rasp(class l_uospodr_data *data);
int  l_uospodr_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uospodr_create_list (class l_uospodr_data *data);

int l_uospodr_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uospodr_p(class l_uospodr_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uospodr_data *data);


extern SQL_baza  bd;

int   l_uospodr(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список подразделений"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uospodr_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[FK7], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uospodr_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uospodr_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uospodr_vibor),data);
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uospodr_prov_row(row,data) != 0)
    continue;
...
l_uospodr_add_columns (GTK_TREE_VIEW (data->treeview));
...
//g_print("l_uospodr_knopka knop=%d\n",knop);
data->kl_shift=0; //0-отжата 1-нажата  

switch (knop)
 {
  case FK2:
    l_uospodr_v(&data->kod_podr_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_uospodr_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_uospodr_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uospodr_create_list(data);
...
if(l_uospodr_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uospod where kod=%d",data->kodv.ravno_atoi());
...
sprintf(strsql,"upodr%d.lst",getpid());
...
iceb_zagolov(gettext("Список подразделений"),0,0,0,0,0,0,ff,data->window);
...
int  l_uospodr_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];
...
sprintf(strsql,"select podr from Uosdok1 where podr=%s limit 1",kod);
...
sprintf(strsql,"select podr from Uosamor1 where podr=%s limit 1",kod);
...
char strsql[512];
sprintf(strsql,"select kod from Uospod");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uospodr_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uospod where kod=%d",atoi(row[0]));
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
 }
