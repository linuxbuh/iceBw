/*$Id: l_xdkop_v.c,v 1.21 2013/10/04 07:27:16 sasa Exp $*/
/*02.03.2016	18.01.2006	Белых А.И.	l_xdkop_v.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_xdkop_v_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_xdkop_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_xdkop_v_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_xdkop_v_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_xdkop_v_v_knopka),&data);
