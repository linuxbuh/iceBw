/*$Id: l_uosgrnu_v.c,v 1.20 2013/08/25 08:26:40 sasa Exp $*/
/*01.03.2016	18.10.2007	Белых А.И.	l_uosgrnu_v.c
  ...
  void read_rek()
   {
    ...
    rk.metka_ta=gtk_combo_box_get_active (GTK_COMBO_BOX(opt));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosgrnu_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_uosgrnu_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_uosgrnu_v_vvod),&data);
...
g_signal_connect(data.entry[E_PROC], "activate",G_CALLBACK(l_uosgrnu_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_uosgrnu_v_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F10 %s",gettext("Выход"));
data.knopka[FK10]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK10],gettext("Завершение работы с меню без записи введенной информации"));
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_uosgrnu_v_knopka),&data);
