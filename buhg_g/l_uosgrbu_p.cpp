/*$Id: l_uosgrbu_p.c,v 1.12 2013/08/25 08:26:40 sasa Exp $*/
/*01.03.2016	18.10.2007	Белых А.И.	l_uosgrbu_p.c
...
void l_uosgrbu_p_clear(class l_uosgrbu_p_data *data);
void    l_uosgrbu_p_vvod(GtkWidget *widget,class l_uosgrbu_p_data *data);
void  l_uosgrbu_p_knopka(GtkWidget *widget,class l_uosgrbu_p_data *data);
gboolean   l_uosgrbu_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_uosgrbu_p_data *data);



int l_uosgrbu_p(class l_uosgrbu_rek *rek_poi,GtkWidget *wpredok)
{
class l_uosgrbu_p_data data;
char strsql[512];
int gor=0,ver=0;
data.rk=rek_poi;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosgrbu_p_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_uosgrbu_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_uosgrbu_p_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_uosgrbu_p_knopka),&data);
gtk_widget_set_name(data.knopka[FK2],iceb_u_inttochar(FK2));

sprintf(strsql,"F4 %s",gettext("Очистить"));
data.knopka[FK4]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK4],gettext("Очистить меню от введеноой информации"));
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(l_uosgrbu_p_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_uosgrbu_p_knopka),&data);
