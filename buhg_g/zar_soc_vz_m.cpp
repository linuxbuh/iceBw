/*$Id: zar_soc_vz_m.c,v 1.14 2013/08/25 08:27:01 sasa Exp $*/
/*13.07.2015	16.01.2007	Белых А.И.	zar_soc_vz_m.c
Меню для расчёта свода отчислений на выплаченную зарплату
*/
#include "buhg_g.h"
  ...
  void read_rek()
   {
    ...
    rk->vs_mes.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_VS_MES])));
...
gboolean   zar_soc_vz_m_key_press(GtkWidget *widget,GdkEventKey *event,class zar_soc_vz_m_data *data);
void  zar_soc_vz_m_knopka(GtkWidget *widget,class zar_soc_vz_m_data *data);
void    zar_soc_vz_m_vvod(GtkWidget *widget,class zar_soc_vz_m_data *data);


void  zar_soc_vz_m_e_knopka(GtkWidget *widget,class zar_soc_vz_m_data *data);
...
sprintf(strsql,"%s %s",iceb_get_namesystem(),gettext("Распечатать расчёт отчислений на выданную зарплату"));
label=gtk_label_new(gettext("Распечатать расчёт отчислений на выданную зарплату"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_soc_vz_m_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(zar_soc_vz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(zar_soc_vz_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(zar_soc_vz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(zar_soc_vz_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_TABNOM],"clicked",G_CALLBACK(zar_soc_vz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_TABNOM], "activate",G_CALLBACK(zar_soc_vz_m_vvod),&data);
...
data.knopka_enter[E_VS_MES]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_VS_MES]), data.knopka_enter[E_VS_MES], FALSE, FALSE, 0);
g_signal_connect(data.knopka_enter[E_VS_MES],"clicked",G_CALLBACK(zar_soc_vz_m_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_VS_MES],iceb_u_inttochar(E_VS_MES));
gtk_widget_set_tooltip_text(data.knopka_enter[E_VS_MES],gettext("Выбор даты"));

data.entry[E_VS_MES] = gtk_entry_new();
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_VS_MES]),7);
gtk_box_pack_start (GTK_BOX (hbox[E_VS_MES]), data.entry[E_VS_MES], FALSE, FALSE, 0);
g_signal_connect(data.entry[E_VS_MES], "activate",G_CALLBACK(zar_soc_vz_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_VS_MES]),data.rk->vs_mes.ravno());
gtk_widget_set_name(data.entry[E_VS_MES],iceb_u_inttochar(E_VS_MES));
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(zar_soc_vz_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(zar_soc_vz_m_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zar_soc_vz_m_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(zar_soc_vz_m_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zar_soc_vz_m_knopka),&data);
...
switch (knop)
 {
  ...
  case E_VS_MES:
    iceb_calendar1(&data->rk->vs_mes,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_VS_MES]),data->rk->vs_mes.ravno());
    return;
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zar_soc_vz_m_knopka knop=%d\n",knop);
...
//g_print("zar_soc_vz_m_vvod enter=%d\n",enter);
...
switch (enter)
 {
  ...
  case E_PODR:
    ...
    break;
  case E_VS_MES:
    data->rk->vs_mes.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
