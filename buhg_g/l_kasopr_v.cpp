/*$Id: l_kasopr_v.c,v 1.26 2013/10/04 07:27:16 sasa Exp $*/
/*29.02.2016	18.01.2006	Белых А.И.	l_kasopr_v.c
  ...
  void read_rek()
   {
    ...
    rk.prov=gtk_combo_box_get_active (GTK_COMBO_BOX(opt));
   }
 };
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_kasopr_v_v_key_press),&data);
...
spmenu.plus(gettext("Проводки не нужно делать"));
spmenu.plus(gettext("Проводки нужно делать"));

iceb_pm_vibor(&spmenu,&data.opt,data.rk.prov);
gtk_box_pack_start (GTK_BOX (vbox),data.opt, TRUE, TRUE, 0);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_kasopr_v_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_kasopr_v_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(kasopr_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(l_kasopr_v_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHETK],"clicked",G_CALLBACK(kasopr_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHETK], "activate",G_CALLBACK(l_kasopr_v_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_CN],"clicked",G_CALLBACK(kasopr_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_CN], "activate",G_CALLBACK(l_kasopr_v_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_kasopr_v_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_kasopr_v_v_knopka),&data);
...
sprintf(strsql,"update Prov set oper='%s' where oper='%s' and tz=2 and kto='%s'",nkod,skod,ICEB_MP_KASA);
if(sql_zap(&bd,strsql) != 0)
  iceb_msql_error(&bd,gettext("Ошибка корректировки записи !"),strsql,wpredok);
