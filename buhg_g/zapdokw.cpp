/* $Id: zapdokw.c,v 1.23 2014/07/31 07:08:27 sasa Exp $ */
/*13.07.2015    14.04.1997      Белых А.И. zapdokw.c
Запись накладной
...
//  pnds=iceb_u_atof(bros);
  ...
  data->nomdok_p.ravno_filtr(),dpnn,mpnn,gpnn,metka_opl,data->k00.ravno(),pnds,wpredok) != 0)
    return(1);
  ...
  if(data->sklad_par != 0 && data->tipz == 2)
   {
    ...
    data->nomdok.ravno_filtr(),0,0,0,metka_opl,data->k00.ravno(),pnds,wpredok);
   }
...
if(data->dd_kor != 0 && mz0 == 0)
 {   
   ...
   if(iceb_sql_readkey(strsql,&row,&cur,wpredok) == 1)
    {
     if(mu_pvkuw(atoi(row[0]),wpredok) != 0) /*проверка возможности корректировки записи*/
      return(1);
    }
...
pod=%d and kto='%s'",
  ...
  data->nomdok_kor.ravno_filtr(),data->skl_kor,ICEB_MP_MATU);
  if(sql_zap(&bd,strsql) != 0)
   {
...
pod=%s and kto='%s' and kodkon='%s'",
    ...
    datad.ravno(),
    data->nomdok.ravno(),data->sklad.ravno(),ICEB_MP_MATU,data->kontr_kor.ravno_filtr());
    if(sql_zap(&bd,strsql) != 0)
     {
      sprintf(bros,"zapdokw %s",gettext("Ошибка корректировки записей !"));
      iceb_msql_error(&bd,bros,strsql,wpredok);
