/*$Id: zar_srinu_m.c,v 1.16 2013/08/25 08:27:01 sasa Exp $*/
/*13.07.2015	28.11.2006	Белых А.И.	zar_srinu_m.c
Меню для расчёта сприска работников имеющих начисление или удержание
*/
#include "buhg_g.h"
#include "zar_srinu.h"
enum
...
gboolean   zar_srinu_m_key_press(GtkWidget *widget,GdkEventKey *event,class zar_srinu_m_data *data);
void  zar_srinu_m_knopka(GtkWidget *widget,class zar_srinu_m_data *data);
void    zar_srinu_m_vvod(GtkWidget *widget,class zar_srinu_m_data *data);


void  zar_srinu_m_e_knopka(GtkWidget *widget,class zar_srinu_m_data *data);
...
label=gtk_label_new(gettext("Распечатать список работников имеющих начисление или удержание"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_srinu_m_key_press),&data);
...
data.radio1[0]=gtk_radio_button_new_with_label_from_widget(NULL,gettext("Начисления"));
data.radio1[1]=gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(data.radio1[0]),gettext("Удержания"));

gtk_box_pack_start (GTK_BOX (vbox), data.radio1[0], TRUE, TRUE, 1);
gtk_box_pack_start (GTK_BOX (vbox), data.radio1[1], TRUE, TRUE, 1);

gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(data.radio1[data.rk->metka_pr]),TRUE); //Устанавливем активной кнопку
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_TABNOM],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_TABNOM], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_NAH],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_NAH], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_UD],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_UD], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_KAT],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_KAT], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
...
data.knopka_enter[E_V_MES]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_V_MES]), data.knopka_enter[E_V_MES], FALSE, FALSE, 0);
g_signal_connect(data.knopka_enter[E_V_MES],"clicked",G_CALLBACK(zar_srinu_m_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_V_MES],iceb_u_inttochar(E_V_MES));
gtk_widget_set_tooltip_text(data.knopka_enter[E_V_MES],gettext("Выбор даты"));

data.entry[E_V_MES] = gtk_entry_new ();
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_V_MES]),7);
gtk_box_pack_start (GTK_BOX (hbox[E_V_MES]), data.entry[E_V_MES], FALSE, FALSE, 0);
g_signal_connect(data.entry[E_V_MES], "activate",G_CALLBACK(zar_srinu_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_V_MES]),data.rk->v_mes.ravno());
gtk_widget_set_name(data.entry[E_V_MES],iceb_u_inttochar(E_V_MES));
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zar_srinu_m_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(zar_srinu_m_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zar_srinu_m_knopka),&data);
...
switch (knop)
 {
  ...
  case E_V_MES:
    iceb_calendar1(&data->rk->v_mes,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_V_MES]),data->rk->v_mes.ravno());
    return;
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zar_srinu_m_knopka knop=%d\n",knop);
...
switch (knop)
 {
  case FK2:
    ...
    if(data->rk->v_mes.prov_dat1() != 0)
     {
      iceb_menu_soob(gettext("Неправильно введена дата (в счёт какого месяца) !"),data->window);
      return;
     }
...
//g_print("zar_srinu_m_vvod enter=%d\n",enter);
...
switch (enter)
 {
  case E_DATAN:
    ...
    break;
  case E_V_MES:
    data->rk->v_mes.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
  case E_DATAK:
