/*$Id: l_vod.c,v 1.9 2013/08/13 06:10:25 sasa Exp $*/
/*12.05.2015	30.05.2009	Белых А.И.	l_vod.c
...
class  l_vod_data
 {
  ...
  int     metka_rr;   //0-работа со списком 1-выбор
  int       metka_radio; //0-на смене 1-все записи
  //Конструктор
  l_vod_data()
   {
    snanomer=0;
...
gboolean   l_vod_key_press(GtkWidget *widget,GdkEventKey *event,class l_vod_data *data);
void l_vod_vibor(GtkTreeSelection *selection,class l_vod_data *data);
void l_vod_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_vod_data *data);
void  l_vod_knopka(GtkWidget *widget,class l_vod_data *data);
void l_vod_add_columns (GtkTreeView *treeview);
void l_vod_udzap(class l_vod_data *data);
int  l_vod_prov_row(SQL_str row,class l_vod_data *data);
void l_vod_rasp(class l_vod_data *data);
int  l_vod_pvu(int,const char *kod,GtkWidget *wpredok);
void l_vod_create_list (class l_vod_data *data);

int l_vod_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_vod_p(class l_vod_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_vod_data *data);

void  l_vod_radio0(GtkWidget *widget,class l_vod_data *data);
void  l_vod_radio1(GtkWidget *widget,class l_vod_data *data);
void l_vod_usm(class l_vod_data *data);
...
int   l_vod(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",name_system,"Список водителей");
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_vod_key_press),&data);
...
data.label_kolstr=gtk_label_new ("Список водителей");
...
GtkWidget *hboxradio=gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 2);
...
gtk_box_pack_start(GTK_BOX(vbox2), hboxradio, FALSE, TRUE, 0);
//Вставляем радиокнопки
GSList *group;

data.radiobutton0=gtk_radio_button_new_with_label(NULL,"Все водители");
gtk_box_pack_start (GTK_BOX (hboxradio),data.radiobutton0, TRUE, TRUE, 0);
g_signal_connect(data.radiobutton0, "clicked",G_CALLBACK(l_vod_radio0),&data);
...
data.radiobutton1=gtk_radio_button_new_with_label(group,"Водители на смене");
gtk_box_pack_start (GTK_BOX (hboxradio),data.radiobutton1, TRUE, TRUE, 0);
g_signal_connect(data.radiobutton1, "clicked",G_CALLBACK(l_vod_radio1),&data);
gtk_widget_set_name(data.radiobutton1,iceb_u_inttochar(1));
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_vod_knopka),&data);
gtk_widget_set_tooltip_text(data.knopka[FK5],gettext("Распечатка записей"));
gtk_widget_set_name(data.knopka[FK5],iceb_u_inttochar(FK5));
gtk_widget_show(data.knopka[FK5]);

sprintf(bros,"F6 %s","Смена");
data.knopka[FK6]=gtk_button_new_with_label(bros);
gtk_box_pack_start(GTK_BOX(vbox1), data.knopka[FK6],TRUE,TRUE, 0);
g_signal_connect(data.knopka[FK6], "clicked",G_CALLBACK(l_vod_knopka),&data);
gtk_widget_set_tooltip_text(data.knopka[FK6],"Установить/снять отметку водителя на смене");
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_vod_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_vod_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_vod_vibor),data);
...
if(data->metka_radio == 0)
  sprintf(strsql,"select * from Taxivod where sm=0 order by kod asc");
else
  sprintf(strsql,"select * from Taxivod order by kod asc");

/*printf("metka_radio=%d %s\n",data->metka_radio,strsql);*/
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  //Дата и время записи
  ss[COL_DATA_VREM].new_plus(iceb_u_vremzap(row[7]));

  //Кто записал
  ss[COL_KTO].new_plus(iceb_u_kszap(row[6],0));
  ...
  COL_KODVOD,ss[COL_KODVOD].ravno(),
  COL_ADRES,ss[COL_ADRES].ravno(),
  COL_TELEF,ss[COL_TELEF].ravno(),
  COL_GOSN,ss[COL_GOSN].ravno(),
...
l_vod_add_columns (GTK_TREE_VIEW (data->treeview));
...
if(data->metka_poi == 1)
 {
  ...
  iceb_str_poisk(&zagolov,data->poisk.kodvod.ravno(),gettext("Код"));
  ...
  iceb_str_poisk(&zagolov,data->poisk.gosn.ravno(),gettext("Гос.номер"));
...
renderer = gtk_cell_renderer_text_new ();
column = gtk_tree_view_column_new_with_attributes (gettext("Код"),renderer,"text",COL_KODVOD,NULL);
gtk_tree_view_column_set_resizable(column,TRUE); /*Разрешение на изменение размеров колонки*/
gtk_tree_view_column_set_sort_column_id (column, COL_KODVOD);
gtk_tree_view_append_column (treeview, column);
...
renderer = gtk_cell_renderer_text_new ();
column = gtk_tree_view_column_new_with_attributes (gettext("Гос.номер"),renderer,"text",COL_GOSN,NULL);
gtk_tree_view_column_set_resizable(column,TRUE); /*Разрешение на изменение размеров колонки*/
gtk_tree_view_column_set_sort_column_id (column, COL_GOSN);
gtk_tree_view_append_column (treeview, column);
...
gtk_tree_model_get(model,&iter,COL_KODVOD,&kod,COL_FIO,&naim,NUM_COLUMNS,&nomer,-1);

data->vodv.new_plus(kod);
...
//g_print("l_vod_knopka knop=%d\n",knop);

switch (knop)
 {
  case FK2:
    if(l_vod_v(&data->kod_kat_tv,data->window) == 0)
      l_vod_create_list(data);
    return;  
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    if(l_vod_v(&data->vodv,data->window) == 0)
      l_vod_create_list(data);
    return;  
  ...
  case FK4:
    if(l_vod_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_vod_create_list(data);
...
{
//printf("l_vod_v_row\n");
//printf("l_vod_v_row корректировка\n");
if(data->metka_rr == 0)
 {
  g_signal_emit_by_name(data->knopka[SFK2],"clicked");
  return;
...
if(l_vod_pvu(1,data->vodv.ravno(),data->window) != 0)
 return;

char strsql[300];

sprintf(strsql,"delete from Taxivod where kod='%s'",data->vodv.ravno());
...
//Полное сравнение
if(iceb_u_proverka(data->poisk.kodvod.ravno(),row[0],0,0) != 0)
 return(1);
...
if(iceb_u_proverka(data->poisk.gosn.ravno(),row[4],4,0) != 0)
 return(1);
...
sprintf(strsql,"taxivod%d.lst",getpid());

imaf.plus(strsql);
naimot.plus("Список водителей");
...
iceb_u_zagolov("Список водителей",0,0,0,0,0,0,iceb_get_pnk("00",0,data->window),ff);

if(data->metka_poi == 1)
 {
  if(data->poisk.kodvod.getdlinna() > 1)
   fprintf(ff,"%s:%s\n",gettext("Код"),data->poisk.kodvod.ravno());
  ...
  if(data->poisk.gosn.getdlinna() > 1)
   fprintf(ff,"%s:%s\n",gettext("Гос. номер"),data->poisk.gosn.ravno());
 }
...
fprintf(ff,"\
Код |    Фамилия         |Дата и время запис.| Кто записал\n");
...
int  l_vod_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[300];

sprintf(strsql,"select kv from Taxizak where kv='%s' limit 1",kod);
...
char strsql[500];
sprintf(strsql,"select kod from Taxivod");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_vod_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Taxivod where kod='%s'",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
...
/************************************************/
/*Установить / снять метку присутствия на смене*/
/***********************************************/
...
SQLCURSOR cur;
short smena=0;
sprintf(strsql,"select sm from Taxivod where kod='%s'",data->vodv.ravno());
...
sprintf(strsql,"update Taxivod set sm=%d where kod='%s'",smena,data->vodv.ravno());
