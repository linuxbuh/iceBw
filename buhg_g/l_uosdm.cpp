/*$Id: l_uosdm.c,v 1.19 2013/12/31 11:49:15 sasa Exp $*/
/*08.05.2015	01.03.2009	Белых А.И.	l_uosdm.c
Работа со справочником драгоценных металлов
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_uosdm.h"
...
gboolean   l_uosdm_key_press(GtkWidget *widget,GdkEventKey *event,class l_uosdm_data *data);
void l_uosdm_vibor(GtkTreeSelection *selection,class l_uosdm_data *data);
void l_uosdm_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_uosdm_data *data);
void  l_uosdm_knopka(GtkWidget *widget,class l_uosdm_data *data);
void l_uosdm_add_columns (GtkTreeView *treeview);
void l_uosdm_udzap(class l_uosdm_data *data);
int  l_uosdm_prov_row(SQL_str row,class l_uosdm_data *data);
void l_uosdm_rasp(class l_uosdm_data *data);
int  l_uosdm_pvu(int,const char *kod,GtkWidget *wpredok);
void l_uosdm_create_list (class l_uosdm_data *data);

int l_uosdm_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_uosdm_p(class l_uosdm_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_uosdm_data *data);


extern SQL_baza  bd;

int   l_uosdm(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список драгоценных металлов"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_uosdm_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_uosdm_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_uosdm_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_uosdm_vibor),data);
...
sprintf(strsql,"select * from Uosdm order by kod asc");
...
//  printf("%s %s %s %s\n",row[0],row[1],row[2],row[3]);
  ...
  if(l_uosdm_prov_row(row,data) != 0)
    continue;
...
l_uosdm_add_columns (GTK_TREE_VIEW (data->treeview));
...
//g_print("l_uosdm_knopka knop=%d\n",knop);
data->kl_shift=0; //0-отжата 1-нажата  

switch (knop)
 {
  case FK2:
    if(l_uosdm_v(&data->kod_kat_tv,data->window) == 0)
      l_uosdm_create_list(data);
    return;  
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    if(l_uosdm_v(&data->kodv,data->window) == 0)
      l_uosdm_create_list(data);
    return;  
  ...
  case FK4:
    if(l_uosdm_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_uosdm_create_list(data);
...
if(l_uosdm_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Uosdm where kod=%d",data->kodv.ravno_atoi());
...
sprintf(strsql,"uosdm%d.lst",getpid());
...
iceb_zagolov(gettext("Список драгоценных металлов"),ff,data->window);
...
int  l_uosdm_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select innom from Uosindm where kod=%d limit 1",atoi(kod));
...
char strsql[512];
sprintf(strsql,"select kod from Uosdm");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_uosdm_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Uosdm where kod=%s",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
