/*$Id: zar_podr_m.c,v 1.14 2013/08/25 08:27:00 sasa Exp $*/
/*13.07.2015	29.01.2007	Белых А.И.	zar_podr_m.c
Меню для расчёта свода по подразделениям форма 1 и 2
*/
#include "buhg_g.h"
#include "zar_podr.h"
enum
...
gboolean   zar_podr_m_key_press(GtkWidget *widget,GdkEventKey *event,class zar_podr_m_data *data);
void  zar_podr_m_knopka(GtkWidget *widget,class zar_podr_m_data *data);
void    zar_podr_m_vvod(GtkWidget *widget,class zar_podr_m_data *data);


void  zar_podr_m_e_knopka(GtkWidget *widget,class zar_podr_m_data *data);

extern SQL_baza  bd;

int zar_podr_m(int metka_r, //0-расчёт формы 1 1-расчёт формы 2
class zar_podr_rek *rek,GtkWidget *wpredok)
{

class zar_podr_m_data data;
char strsql[512];
iceb_u_str kikz;
data.rk=rek;
...
if(metka_r == 0)
 {
  ...
  label=gtk_label_new(gettext("Распечатать отчёты по подразделениям (форма 1)"));
 }
else
 {
  sprintf(strsql,"%s %s",iceb_get_namesystem(),gettext("Распечатать отчёты по подразделениям (форма 2)"));
  label=gtk_label_new(gettext("Распечатать отчёты по подразделениям (форма 2)"));
 }
 
gtk_window_set_title(GTK_WINDOW(data.window),strsql);
g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_podr_m_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_TABNOM],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_TABNOM], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_NAH],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_NAH], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_UD],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_UD], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KOD_KAT],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_KOD_KAT], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(zar_podr_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(zar_podr_m_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zar_podr_m_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(zar_podr_m_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zar_podr_m_knopka),&data);
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zar_podr_m_knopka knop=%d\n",knop);
...
//g_print("zar_podr_m_vvod enter=%d\n",enter);
