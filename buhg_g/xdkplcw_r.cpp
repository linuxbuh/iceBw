/*$Id: xdkplcw_r.c,v 1.5 2013/09/26 09:46:59 sasa Exp $*/
/*23.05.2016	16.10.2012	Белых А.И.	xdkplcw_r.c
Расчёт отчёта 
*/
#include <errno.h>
#include "buhg_g.h"

class xdkplcw_r_data
 {
...
gboolean   xdkplcw_r_key_press(GtkWidget *widget,GdkEventKey *event,class xdkplcw_r_data *data);
gint xdkplcw_r1(class xdkplcw_r_data *data);
void  xdkplcw_r_v_knopka(GtkWidget *widget,class xdkplcw_r_data *data);


extern SQL_baza bd;
extern int      kol_strok_na_liste;

int xdkplcw_r(int metka_prov,class iceb_u_spisok *imaf,class iceb_u_spisok *naimf,GtkWidget *wpredok)
{
char strsql[1024];
class iceb_u_str repl;
class xdkplcw_r_data data;
int gor=0;
int ver=0;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(xdkplcw_r_key_press),&data);
...
g_signal_connect(data.knopka,"clicked",G_CALLBACK(xdkplcw_r_v_knopka),&data);
...
g_idle_add((GSourceFunc)xdkplcw_r1,&data);
...
fprintf(ff_prot,"\n%s\n",gettext("Подсистема \"Платёжные документы\""));
