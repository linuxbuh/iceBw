...
/*23.06.2020	13.01.2008	Белых А.И.	uosdpd_m.c
...
data.knopka_enter[E_IN_NOM]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_IN_NOM]), data.knopka_enter[E_IN_NOM], FALSE, FALSE,1);
g_signal_connect(data.knopka_enter[E_IN_NOM],"clicked",G_CALLBACK(uosdpd_v_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_IN_NOM],iceb_u_inttochar(E_IN_NOM));
gtk_widget_set_tooltip_text(data.knopka_enter[E_IN_NOM],gettext("Выбрать инвентарный номер"));

data.entry[E_IN_NOM] = gtk_entry_new();
gtk_box_pack_start (GTK_BOX (hbox[E_IN_NOM]), data.entry[E_IN_NOM], TRUE, TRUE,1);
g_signal_connect(data.entry[E_IN_NOM], "activate",G_CALLBACK(uosdpd_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_IN_NOM]),data.rk->in_nom.ravno());
gtk_widget_set_name(data.entry[E_IN_NOM],iceb_u_inttochar(E_IN_NOM));
...
switch (knop)
 {
  ...
  case E_IN_NOM:
    if(l_uosin(1,&kod,&naim,data->window) == 0)
      data->rk->in_nom.z_plus(kod.ravno());
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_IN_NOM]),data->rk->in_nom.ravno());
...
switch (enter)
 {
  ...
  case E_GRUP_BU:
    ...
    break;
  case E_IN_NOM:
    data->rk->in_nom.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
