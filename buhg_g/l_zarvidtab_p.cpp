/*$Id: l_zarvidtab_p.c,v 1.13 2013/08/25 08:26:47 sasa Exp $*/
/*03.03.2016	23.06.2006	Белых А.И.	l_zarvidtab_p.c
...
class l_zarvidtab_p_data
 {
  ...
  l_zarvidtab_p_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
void l_zarvidtab_p_clear(class l_zarvidtab_p_data *data);
void    l_zarvidtab_p_vvod(GtkWidget *widget,class l_zarvidtab_p_data *data);
void  l_zarvidtab_p_knopka(GtkWidget *widget,class l_zarvidtab_p_data *data);
gboolean   l_zarvidtab_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_zarvidtab_p_data *data);


int l_zarvidtab_p(class l_zarvidtab_rek *rek_poi,GtkWidget *wpredok)
{
class l_zarvidtab_p_data data;
char strsql[512];
int gor=0,ver=0;
data.rk=rek_poi;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_zarvidtab_p_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_zarvidtab_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_zarvidtab_p_vvod),&data);
...
g_signal_connect(data.knopka[PFK2],"clicked",G_CALLBACK(l_zarvidtab_p_knopka),&data);
...
g_signal_connect(data.knopka[PFK4],"clicked",G_CALLBACK(l_zarvidtab_p_knopka),&data);
...
g_signal_connect(data.knopka[PFK10],"clicked",G_CALLBACK(l_zarvidtab_p_knopka),&data);
