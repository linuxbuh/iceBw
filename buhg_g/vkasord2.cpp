/*$Id: vkasord2.c,v 1.25 2013/10/04 07:27:17 sasa Exp $*/
/*13.07.2015	24.01.2006	Белых А.И.	vkasord2.c
...
class vkasord2_data
 {
  ...
  void read_rek()
   {
    kassa.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_KASSA])));
    ...
    osnov.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_OSNOV])));
    dopol.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_DOPOL])));
    fio.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_FIO])));
    dokum.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_DOKUM])));
    nomer_bl.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_NOMER_BL])));
    nomved.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_NOMVED])));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(vkasord2_v_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_KASSA],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KASSA], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAD],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAD], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_NOMDOK],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_NOMDOK], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SHET],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SHET], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KODOP],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KODOP], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.entry[E_OSNOV], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.entry[E_DOPOL], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.entry[E_FIO], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_DOKUM]),255);
...
g_signal_connect(data.entry[E_DOKUM], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
g_signal_connect(data.entry[E_NOMER_BL], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
...
data.knopka_enter[E_NOMVED]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_NOMVED]), data.knopka_enter[E_NOMVED], FALSE, FALSE, 0);
g_signal_connect(data.knopka_enter[E_NOMVED],"clicked",G_CALLBACK(vkasord2_v_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_NOMVED],iceb_u_inttochar(E_NOMVED));
gtk_widget_set_tooltip_text(data.knopka_enter[E_NOMVED],gettext("Получение номера ведомости"));

data.entry[E_NOMVED] = gtk_entry_new ();
gtk_entry_set_max_length(GTK_ENTRY(data.entry[E_NOMVED]),20);
gtk_box_pack_start (GTK_BOX (hbox[E_NOMVED]), data.entry[E_NOMVED],TRUE, TRUE, 0);
g_signal_connect(data.entry[E_NOMVED], "activate",G_CALLBACK(vkasord2_v_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_NOMVED]),data.nomved.ravno());
gtk_widget_set_name(data.entry[E_NOMVED],iceb_u_inttochar(E_NOMVED));


sprintf(strsql,"F2 %s",gettext("Запись"));
data.knopka[FK2]=gtk_button_new_with_label(strsql);
gtk_widget_set_tooltip_text(data.knopka[FK2],gettext("Запись введённой в меню информации."));
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(vkasord2_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(vkasord2_v_knopka),&data);
