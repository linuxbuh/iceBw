/*$Id: l_sval.c,v 1.2 2013/12/31 11:49:14 sasa Exp $*/
/*08.05.2015	05.09.2013	Белых А.И.	l_sval.c
Работа со списком валют
*/
#include        <errno.h>
#include  "buhg_g.h"
#include  "l_sval.h"
...
gboolean   l_sval_key_press(GtkWidget *widget,GdkEventKey *event,class l_sval_data *data);
void l_sval_vibor(GtkTreeSelection *selection,class l_sval_data *data);
void l_sval_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class l_sval_data *data);
void  l_sval_knopka(GtkWidget *widget,class l_sval_data *data);
void l_sval_add_columns (GtkTreeView *treeview);
void l_sval_udzap(class l_sval_data *data);
int  l_sval_prov_row(SQL_str row,class l_sval_data *data);
void l_sval_rasp(class l_sval_data *data);
int  l_sval_pvu(int,const char *kod,GtkWidget *wpredok);
void l_sval_create_list (class l_sval_data *data);

int l_sval_v(class iceb_u_str *kod_gr,GtkWidget *wpredok);
int l_sval_p(class l_sval_rek *rek_poi,GtkWidget *wpredok);
void l_gruppdok_unk(class l_sval_data *data);


extern SQL_baza  bd;

int l_sval(int metka_rr, //0-ввод и корек. 1-выбор
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Справочник валют"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_sval_key_press),&data);
...
data.label_kolstr=gtk_label_new (gettext("Справочник валют"));
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(l_sval_knopka),&data);
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(l_sval_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(l_sval_vibor),data);
...
sprintf(strsql,"select * from Glksval order by kod asc");
...
l_sval_add_columns (GTK_TREE_VIEW (data->treeview));
...
iceb_u_str stroka;
iceb_u_str zagolov;
zagolov.plus(gettext("Справочник валют"));
...
//g_print("l_sval_knopka knop=%d\n",knop);
data->kl_shift=0; //0-отжата 1-нажата  

switch (knop)
 {
  case FK2:
    l_sval_v(&data->kod_kat_tv,data->window);
  ...
  case SFK2:
    if(data->kolzap == 0)
      return;
    l_sval_v(&data->kodv,data->window);
  ...
  case FK4:
    if(l_sval_p(&data->poisk,data->window) == 0)
     data->metka_poi=1;
    else
     data->metka_poi=0;
    l_sval_create_list(data);
...
if(l_sval_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Glksval where kod='%s'",data->kodv.ravno());
...
sprintf(strsql,"sval%d.lst",getpid());

imaf.plus(strsql);
naimot.plus(gettext("Справочник валют"));
...
iceb_zagolov(gettext("Справочник валют"),0,0,0,0,0,0,ff,data->window);
...
int  l_sval_pvu(int met,const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kod from Glkkv where kod='%s' limit 1",kod);
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
    sprintf(strsql,"%s Glkkv",gettext("Удалить невозможно ! Есть записи в таблице"));
    iceb_menu_soob(strsql,wpredok);
   }
  return(1);
 }    

sprintf(strsql,"select kod from Kas where kv='%s' limit 1",kod);
if(iceb_sql_readkey(strsql,wpredok) > 0)
 {
  if(met != 0)
   {
...
char strsql[512];
sprintf(strsql,"select kod from Glksval");
int kolstr=0;
if((kolstr=cur.make_cursor(&bd,strsql)) < 0)
 {
  iceb_msql_error(&bd,gettext("Ошибка создания курсора !"),strsql,data->window);
...
while(cur.read_cursor(&row) != 0)
 {
  if(l_sval_pvu(0,row[0],data->window) != 0)
   continue;
  sprintf(strsql,"delete from Glksval where kod='%s'",row[0]);
  if(iceb_sql_zapis(strsql,0,0,data->window) != 0)
   break;
  koludzap++;
