/*$Id: ukrplcw_r.c,v 1.6 2013/09/26 09:46:56 sasa Exp $*/
/*23.05.2016	11.10.2012	Белых А.И.	ukrplcw_r.c
Расчёт отчёта 
*/
#include <errno.h>
#include "buhg_g.h"

class ukrplcw_r_data
 {
...
gboolean   ukrplcw_r_key_press(GtkWidget *widget,GdkEventKey *event,class ukrplcw_r_data *data);
gint ukrplcw_r1(class ukrplcw_r_data *data);
void  ukrplcw_r_v_knopka(GtkWidget *widget,class ukrplcw_r_data *data);


extern SQL_baza bd;
extern int      kol_strok_na_liste;

int ukrplcw_r(int metka_prov,class iceb_u_spisok *imaf,class iceb_u_spisok *naimf,GtkWidget *wpredok)
{
char strsql[1024];
class iceb_u_str repl;
class ukrplcw_r_data data;
int gor=0;
int ver=0;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(ukrplcw_r_key_press),&data);
...
g_signal_connect(data.knopka,"clicked",G_CALLBACK(ukrplcw_r_v_knopka),&data);
...
g_idle_add((GSourceFunc)ukrplcw_r1,&data);
  ...
  if(iceb_sql_readkey(strsql,data->window) == 0)
   {
    ...
    fprintf(ff_prot,"%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s|%s\n",row[0],row[1],row[2],row[3],row[4],row[5],row[6],row[7],row[8],row[9],row[10],row[11],row[12],row[13],row[14],row[15],row[16]);
    fprintf(ff_prot,"%s\n",strsql);
    fprintf(ff_prot,"%s!\n",gettext("Не найдена шапка документа"));

    if(data->metka_prov == 1)
     {
      sprintf(strsql,"delete from Ukrdok1 where kassa=%d and god=%d and tp=%d and nomd='%s' and kontr='%s'",
