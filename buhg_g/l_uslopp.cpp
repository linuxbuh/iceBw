/*$Id: l_uslopp.c,v 1.37 2013/08/25 08:26:43 sasa Exp $*/
/*08.05.2015	17.07.2005	Белых А.И.	l_uslopp.c
Ввод и корректировка списка операций прихода для подсистемы "Учёт услуг"
*/
#include <errno.h>
#include "buhg_g.h"
#include "l_uslop.h"
...
gboolean   uslopp_key_press(GtkWidget *widget,GdkEventKey *event,class uslopp_data *data);
void uslopp_vibor(GtkTreeSelection *selection,class uslopp_data *data);
void uslopp_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,class uslopp_data *data);
void  uslopp_knopka(GtkWidget *widget,class uslopp_data *data);
void uslopp_add_columns (GtkTreeView *treeview);
void uslopp_create_list(class uslopp_data *data);
int  uslopp_prov_row(SQL_str row,class uslopp_data *data);

int l_uslopp_v(iceb_u_str *kodzap,GtkWidget *wpredok);
int l_uslopp_p(class uslop_rek *datap,GtkWidget *wpredok);
int  uslopp_pvu(int met,const char *kod,GtkWidget *wpredok);
void uslopp_udzap(class uslopp_data *data);
void uslopp_unk(GtkWidget *wpredok);
void uslopp_rasp(class uslopp_data *data);

extern SQL_baza	bd;
 
int l_uslopp(int metka, //0-воод и корректировка 1-выбор
iceb_u_str *kod,iceb_u_str *naimk,GtkWidget *wpredok)
{
int gor=0;
int ver=0;
class uslopp_data data;
char bros[512];
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(uslopp_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(uslopp_knopka),&data);
...
switch (knop)
 {
  case FK2:
    data->kodtv.new_plus("");
    l_uslopp_v(&data->kodtv,data->window);
    uslopp_create_list(data);
    return;  

  case SFK2:

    if(l_uslopp_v(&data->kodv,data->window) == 0)
...
//    printf("data->snanomer=%d\n",data->snanomer);
  ...
  case SFK3:
    uslopp_unk(data->window);
    uslopp_create_list(data);
    return;  

  case FK4:
    l_uslopp_p(&data->poi,data->window);
    uslopp_create_list(data);
    return;  
...
{
iceb_u_str repl;
//printf("uslopp_key_press keyval=%d state=%d\n",event->keyval,event->state);
...
switch(event->keyval)
 {
  ...
  case ICEB_REG_L:
  case ICEB_REG_R:
    printf("uslopp_key_press-Нажата клавиша Shift\n");
...
g_signal_connect(data->treeview,"row_activated",G_CALLBACK(uslopp_v_row),data);

GtkTreeSelection *selection=gtk_tree_view_get_selection(GTK_TREE_VIEW(data->treeview));
gtk_tree_selection_set_mode(selection,GTK_SELECTION_SINGLE);
g_signal_connect(selection,"changed",G_CALLBACK(uslopp_vibor),data);
...
uslopp_add_columns (GTK_TREE_VIEW (data->treeview));
...
//printf("uslopp_vibor-%s %d\n",data->kodv.ravno(),data->snanomer);

}
/****************************/
/*Выбор строки по двойному клику мышкой*/
/**********************/
void uslopp_v_row(GtkTreeView *treeview,GtkTreePath *arg1,GtkTreeViewColumn *arg2,
class uslopp_data *data)
{
//printf("uslopp_v_row\n");
//printf("uslopp_v_row корректировка\n");
...
if(uslopp_pvu(1,data->kodv.ravno(),data->window) != 0)
 return;

char strsql[512];

sprintf(strsql,"delete from Usloper1 where kod='%s'",
data->kodv.ravno());
...
SQLCURSOR cur;
int kolstr;
sprintf(strsql,"select kod from Usloper1");
...
while(cur.read_cursor(&row) != 0)
 {
  if(uslopp_pvu(0,row[0],wpredok) != 0)
    continue;
  sprintf(strsql,"delete from Usloper1 where kod='%s'",row[0]);
...
sprintf(strsql,"uslopp%d.lst",getpid());
