/*$Id: doocsumw_m.c,v 1.18 2013/09/26 09:46:47 sasa Exp $*/
/*10.07.2015	06.05.2005	Белых А.И.	doocsumw_m.c
Меню для ввода реквизитов расчёта сумм выполненых дооценок
*/
#include "buhg_g.h"
#include "doocsumw.h"
enum
...
gboolean   doocsumw_v_key_press(GtkWidget *widget,GdkEventKey *event,class doocsumw_data *data);
void    doocsumw_v_vvod(GtkWidget *widget,class doocsumw_data *data);
void  doocsumw_v_knopka(GtkWidget *widget,class doocsumw_data *data);
void  doocsumw_v_e_knopka(GtkWidget *widget,class doocsumw_data *data);
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(doocsumw_v_key_press),&data);

GtkWidget *label=gtk_label_new(gettext("Распечатать суммы выполненных автоматических дооценок"));
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(doocsumw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(doocsumw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(doocsumw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(doocsumw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_KODOP],"clicked",G_CALLBACK(doocsumw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_KODOP], "activate",G_CALLBACK(doocsumw_v_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_SKLAD],"clicked",G_CALLBACK(doocsumw_v_e_knopka),&data);
...
g_signal_connect(data.entry[E_SKLAD], "activate",G_CALLBACK(doocsumw_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(doocsumw_v_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(doocsumw_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(doocsumw_v_knopka),&data);
...
switch (knop)
 {
  ...
  case E_KODOP:
    iceb_vibrek(0,"Prihod",&data->rk->kodop,data->window);
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KODOP]),data->rk->kodop.ravno());
    return;
...
//g_print("doocsumw_v_vvod enter=%d\n",enter);
