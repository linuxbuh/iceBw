/*$Id: xuslplcw_r.c,v 1.6 2013/09/26 09:46:59 sasa Exp $*/
/*23.05.2016	12.10.2012	Белых А.И.	xuslplcw_r.c
Расчёт отчёта 
*/
#include <errno.h>
#include "buhg_g.h"

class xuslplcw_r_data
 {
...
gboolean   xuslplcw_r_key_press(GtkWidget *widget,GdkEventKey *event,class xuslplcw_r_data *data);
gint xuslplcw_r1(class xuslplcw_r_data *data);
void  xuslplcw_r_v_knopka(GtkWidget *widget,class xuslplcw_r_data *data);


extern SQL_baza bd;
extern int      kol_strok_na_liste;

int xuslplcw_r(int metka_prov,class iceb_u_spisok *imaf,class iceb_u_spisok *naimf,GtkWidget *wpredok)
{
char strsql[1024];
class iceb_u_str repl;
class xuslplcw_r_data data;
int gor=0;
int ver=0;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(xuslplcw_r_key_press),&data);
...
g_signal_connect(data.knopka,"clicked",G_CALLBACK(xuslplcw_r_v_knopka),&data);
...
g_idle_add((GSourceFunc)xuslplcw_r1,&data);
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(row[1][0] == '0')
   {
    ...
    iceb_printw(strsql,data->buffer,data->view);
    sprintf(strsql,"%s tp %s Usldokum2 %s !!!\n",gettext("Колонка"),gettext("в таблице"),gettext("равна нолю"));
    iceb_printw(strsql,data->buffer,data->view);
  ...
  sprintf(strsql,"select tp from Usldokum1 where datd='%s' and podr=%d and nomd='%s' and tp=%d and metka=%d and kodzap=%d limit 1",
