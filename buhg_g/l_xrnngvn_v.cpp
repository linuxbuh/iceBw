/*$Id: l_xrnngvn_v.c,v 1.16 2013/08/25 08:26:45 sasa Exp $*/
/*02.03.2016	14.04.2008	Белых А.И.	l_xrnngvn_v.c
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_xrnngvn_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_xrnngvn_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_xrnngvn_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_xrnngvn_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_xrnngvn_v_knopka),&data);
...
sprintf(strsql,"update Reenn set gr='%s' where gr='%s'",nkod,skod);
