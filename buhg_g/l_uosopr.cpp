/*$Id: l_uosopr.c,v 1.26 2013/08/25 08:26:41 sasa Exp $*/
/*08.05.2015	28.12.2005	Белых А.И.	l_uosopr.c
...
sprintf(bros,"%s %s",iceb_get_namesystem(),gettext("Список операций расходов"));
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(uosopr_key_press),&data);
...
g_signal_connect(data.knopka[FK2], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[SFK2], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[FK3], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[SFK3], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[FK4], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[FK5], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
g_signal_connect(data.knopka[FK10], "clicked",G_CALLBACK(uosopr_knopka),&data);
...
iceb_u_zagolov(gettext("Список операций расходов"),0,0,0,0,0,0,iceb_get_pnk("00",0,data->window),ff);
...
fprintf(ff,gettext("\
Код |    Наименование операции     |Вид операции|Проводки|Код.конт.|Дата и время запис.| Кто записал\n"));
/********
1234|123456789012345678901234567890 123456789012 12345678 123456789 123456789
**********/
fprintf(ff,"\
---------------------------------------------------------------------------------------------------------------\n");
...
while(cur.read_cursor(&row) != 0)
 {
  ...
  if(row[4][0] == '2')
    s5.new_plus(gettext("Смена стоимости"));
  //Проводки
  if(row[5][0] == '0')
