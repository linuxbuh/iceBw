/*$Id: l_ukrzat_p.c,v 1.13 2013/08/25 08:26:39 sasa Exp $*/
/*01.03.2016	05.05.2004	Белых А.И.	l_ukrzat_p.c
...
class l_ukrzat_p_data
 {
  ...
  class l_ukrzat_rek *rk;
  
  
  l_ukrzat_p_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
void l_ukrzat_p_clear(class l_ukrzat_p_data *data);
void    l_ukrzat_p_vvod(GtkWidget *widget,class l_ukrzat_p_data *data);
void  l_ukrzat_p_knopka(GtkWidget *widget,class l_ukrzat_p_data *data);
gboolean   l_ukrzat_p_key_press(GtkWidget *widget,GdkEventKey *event,class l_ukrzat_p_data *data);


int l_ukrzat_p(class l_ukrzat_rek *rek_poi,GtkWidget *wpredok)
{
class l_ukrzat_p_data data;
char strsql[512];
int gor=0,ver=0;
data.rk=rek_poi;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_ukrzat_p_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_ukrzat_p_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_ukrzat_p_vvod),&data);
...
g_signal_connect(data.knopka[PFK2],"clicked",G_CALLBACK(l_ukrzat_p_knopka),&data);
...
g_signal_connect(data.knopka[PFK4],"clicked",G_CALLBACK(l_ukrzat_p_knopka),&data);
...
g_signal_connect(data.knopka[PFK10],"clicked",G_CALLBACK(l_ukrzat_p_knopka),&data);
