/*$Id: zar_soc_nah_m.c,v 1.13 2013/08/25 08:27:01 sasa Exp $*/
/*10.07.2017	16.01.2007	Белых А.И.	zar_soc_nah_m.c
Меню для расчёта свода отчислений в соц-фонды на фонд оплаты труда
*/
#include "buhg_g.h"
...
class zar_soc_nah_m_data
 {
  ...
  zar_soc_nah_m_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
  ...
  void read_rek()
   {
    ...
    rk->kod_soc.new_plus(gtk_entry_get_text(GTK_ENTRY(entry[E_KOD_SOC])));
...
gboolean   zar_soc_nah_m_key_press(GtkWidget *widget,GdkEventKey *event,class zar_soc_nah_m_data *data);
void  zar_soc_nah_m_knopka(GtkWidget *widget,class zar_soc_nah_m_data *data);
void    zar_soc_nah_m_vvod(GtkWidget *widget,class zar_soc_nah_m_data *data);


void  zar_soc_nah_m_e_knopka(GtkWidget *widget,class zar_soc_nah_m_data *data);
...
label=gtk_label_new(gettext("Расчёт начисления ЕСВ"));

gtk_window_set_title(GTK_WINDOW(data.window),strsql);
g_signal_connect(data.window,"delete_event",G_CALLBACK(gtk_widget_destroy),NULL);
g_signal_connect(data.window,"destroy",G_CALLBACK(gtk_main_quit),NULL);
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(zar_soc_nah_m_key_press),&data);
...
g_signal_connect(data.knopka_enter[E_DATAN],"clicked",G_CALLBACK(zar_soc_nah_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAN], "activate",G_CALLBACK(zar_soc_nah_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_DATAK],"clicked",G_CALLBACK(zar_soc_nah_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_DATAK], "activate",G_CALLBACK(zar_soc_nah_m_vvod),&data);
...
g_signal_connect(data.knopka_enter[E_TABNOM],"clicked",G_CALLBACK(zar_soc_nah_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_TABNOM], "activate",G_CALLBACK(zar_soc_nah_m_vvod),&data);
...
sprintf(strsql,"%s (,,)",gettext("Код соц. фонда"));
data.knopka_enter[E_KOD_SOC]=gtk_button_new_with_label(strsql);
gtk_box_pack_start (GTK_BOX (hbox[E_KOD_SOC]), data.knopka_enter[E_KOD_SOC], FALSE, FALSE, 0);
g_signal_connect(data.knopka_enter[E_KOD_SOC],"clicked",G_CALLBACK(zar_soc_nah_m_e_knopka),&data);
gtk_widget_set_name(data.knopka_enter[E_KOD_SOC],iceb_u_inttochar(E_KOD_SOC));
gtk_widget_set_tooltip_text(data.knopka_enter[E_KOD_SOC],gettext("Выбор кода соц. фонда"));

data.entry[E_KOD_SOC] = gtk_entry_new();
gtk_box_pack_start (GTK_BOX (hbox[E_KOD_SOC]), data.entry[E_KOD_SOC], TRUE, TRUE, 0);
g_signal_connect(data.entry[E_KOD_SOC], "activate",G_CALLBACK(zar_soc_nah_m_vvod),&data);
gtk_entry_set_text(GTK_ENTRY(data.entry[E_KOD_SOC]),data.rk->kod_soc.ravno());
gtk_widget_set_name(data.entry[E_KOD_SOC],iceb_u_inttochar(E_KOD_SOC));
...
g_signal_connect(data.knopka_enter[E_PODR],"clicked",G_CALLBACK(zar_soc_nah_m_e_knopka),&data);
...
g_signal_connect(data.entry[E_PODR], "activate",G_CALLBACK(zar_soc_nah_m_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(zar_soc_nah_m_knopka),&data);
...
g_signal_connect(data.knopka[FK4],"clicked",G_CALLBACK(zar_soc_nah_m_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(zar_soc_nah_m_knopka),&data);
...
switch (knop)
 {
  ...
  case E_KOD_SOC:
    if(l_zarsocf(1,&kod,&naim,data->window) == 0)
     data->rk->kod_soc.z_plus(kod.ravno());
    gtk_entry_set_text(GTK_ENTRY(data->entry[E_KOD_SOC]),data->rk->kod_soc.ravno());
    return;
...
//printf("zar_soc_nah_m_key_press\n");
switch(event->keyval)
 {
...
int knop=atoi(gtk_widget_get_name(widget));
//g_print("zar_soc_nah_m_knopka knop=%d\n",knop);
...
//g_print("zar_soc_nah_m_vvod enter=%d\n",enter);
...
switch (enter)
 {
  ...
  case E_PODR:
    ...
    break;
  case E_KOD_SOC:
    data->rk->kod_soc.new_plus(gtk_entry_get_text(GTK_ENTRY(widget)));
    break;
