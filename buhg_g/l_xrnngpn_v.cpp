/*$Id: l_xrnngpn_v.c,v 1.15 2013/08/25 08:26:44 sasa Exp $*/
/*02.03.2016	14.04.2008	Белых А.И.	l_xrnngpn_v.c
Ввод и корректировка групп полученных налоговых накладных
*/
#include "buhg_g.h"
#include "l_xrnngpn.h"
enum
...
class l_xrnngpn_v_data
 {
  ...
  class l_xrnngpn_rek rk;
  class iceb_u_str kod_gr;  
  
  l_xrnngpn_v_data() //Конструктор
   {
    kl_shift=0;
    voz=1;
   }
...
gboolean   l_xrnngpn_v_key_press(GtkWidget *widget,GdkEventKey *event,class l_xrnngpn_v_data *data);
void  l_xrnngpn_v_knopka(GtkWidget *widget,class l_xrnngpn_v_data *data);
void    l_xrnngpn_v_vvod(GtkWidget *widget,class l_xrnngpn_v_data *data);
int l_xrnngpn_pk(const char *god,GtkWidget*);
int l_xrnngpn_zap(class l_xrnngpn_v_data *data);
void l_xrnngpn_kkvt(const char *kods,const char *kodn,GtkWidget *wpredok);
...
if(data.kod_gr.getdlinna() >  1)
 {
  ...
  sprintf(strsql,"select * from Reegrup1 where kod='%s'",data.kod_gr.ravno());
  SQL_str row;
  SQLCURSOR cur;
...
g_signal_connect_after(data.window,"key_press_event",G_CALLBACK(l_xrnngpn_v_key_press),&data);
...
g_signal_connect(data.entry[E_KOD], "activate",G_CALLBACK(l_xrnngpn_v_vvod),&data);
...
g_signal_connect(data.entry[E_NAIM], "activate",G_CALLBACK(l_xrnngpn_v_vvod),&data);
...
g_signal_connect(data.knopka[FK2],"clicked",G_CALLBACK(l_xrnngpn_v_knopka),&data);
...
g_signal_connect(data.knopka[FK10],"clicked",G_CALLBACK(l_xrnngpn_v_knopka),&data);
...
if(iceb_u_SRAV(data->kod_gr.ravno(),data->rk.kod.ravno(),0) != 0)
  if(l_xrnngpn_pk(data->rk.kod.ravno(),data->window) != 0)
     return(1);

if(data->kod_gr.getdlinna() <= 1)
 {

  sprintf(strsql,"insert into Reegrup1 \
values ('%s','%s',%d,%ld)",
...
else
 {
  ...
  sprintf(strsql,"update Reegrup1 \
set \
kod='%s',\
naik='%s',\
ktoz=%d,\
...
if(metkakor == 1)
 l_xrnngpn_kkvt(data->kod_gr.ravno(),data->rk.kod.ravno(),data->window);
...
/**********************************/
int l_xrnngpn_pk(const char *kod,GtkWidget *wpredok)
{
char strsql[512];

sprintf(strsql,"select kod from Reegrup1 where kod='%s'",kod);
...
void l_xrnngpn_kkvt(const char *skod,const char *nkod,GtkWidget *wpredok)
{

char strsql[512];

sprintf(strsql,"update Reenn1 set gr='%s' where gr='%s'",nkod,skod);
