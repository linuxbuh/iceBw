/*$Id: uoszaphd.c,v 1.16 2013/05/17 14:58:28 sasa Exp $*/
/*16.04.2017	28.10.2007	Белых А.И.	uoszaphd.c
...
kto='%s' and tz=%d",
    ...
    gz,mz,dz,gz,mz,dz,data->nomdok_z.ravno(),data->podr_z.ravno_atoi(),ICEB_MP_UOS,
    data->tipz);
...
where datd='%04d-%d-%d' and nomd='%s' and pod=%d and kto='%s' and tz=%d",
   ...
   gz,mz,dz,data->nomdok_z.ravno(),data->podr_z.ravno_atoi(),ICEB_MP_UOS,
   data->tipz);
...
kodkon='%s' and tz=%d",
    data->kontr.ravno(),
    gz,mz,dz,data->nomdok_z.ravno(),data->podr.ravno_atoi(),ICEB_MP_UOS,data->kontr_z.ravno(),
    data->tipz);
